...
const COPY = {
 ...
 heroLead:
 "GyanQuest is a growing library of 2D mission games - বাংলা and English - that turn textbooks into labs you can touch on any phone.",
 exploreGames: "Explore games",
 whyUs: "Why GyanQuest",
 statGames: "Learning games",
 statLang: "Languages",
 statPlay: "Play style",
 statPlayVal: "2D + missions",
 ...
 heroLead:
 "GyanQuest হলো ২ডি মিশন গেমের বাড়তে থাকা লাইব্রেরি - বাংলা ও ইংরেজিতে - যেখানে পাঠ্যবইয়ের ধারণা ফোনেই স্পর্শ করে শেখা যায়।",
 exploreGames: "গেম দেখুন",
 whyUs: "কেন GyanQuest",
 statGames: "শিক্ষা গেম",
 statLang: "ভাষা",
 statPlay: "খেলার ধরন",
 statPlayVal: "২ডি + মিশন",
 gamesTitle: "গেম লাইব্রেরি",
 gamesLead: "২৮টি লাইভ মিশন গেম - একই শেয়ার্ড ইঞ্জিন, সব বিষয় খেলা যায়।",
 live: "খেলার উপযোগী",
 underDev: "উন্নয়নাধীন",
 soon: "শীঘ্রই আসছে",
...
/** Games with ≥1 uniqueness-pass (Playable) mission - mirrors engine/js/mission-status.js */
const FINISHED_GAME_SLUGS = new Set([
 "chemistry-lab",
 "force-fighter",
 "bio-explorer",
 "math-quest",
...
/** @returns {"playable"|"dev"|"soon"} */
function catalogTier(item) {
 if (!item.live) return "soon";
 const slug = slugFromHref(item.href);
 if (FINISHED_GAME_SLUGS.has(slug)) return "playable";
 return "dev";
}
...
function renderCatalog() {
 ...
 const tier = catalogTier(item);
 const card = document.createElement(item.live ? "a" : "div");
 card.className = `gq-card${
 tier === "playable" ? " gq-card--live" : tier === "dev" ? " gq-card--dev" : " gq-card--locked"
 }`;
 ...
 const badgeText = tier === "playable" ? t("live") : tier === "dev" ? t("underDev") : t("soon");
 card.innerHTML = `
 ...
 <span class="gq-card__badge gq-card__badge--${tier}">${badgeText}</span>`;
...
async function initAuth() {
 ...
 if (!host) return;
 try {
    const auth = await import("/engine/js/auth-api.js?v=auth2");
