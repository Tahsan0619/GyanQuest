...
export function registerServerScenes(arena) {
 ...
 arena.registerScene(P + "Drill", (api) => {
  ...
  setTick(() => {
   ...
   drawBackdrop();
   drawLabel(ctx, labState.prompt || "Server drill", w * 0.5, h * 0.18, { h: 32, font: "700 16px Segoe UI" });
 ...
 arena.registerScene(P + "Mastery", (api) => {
  ...
  setTick(() => {
   ...
   drawReqRes(ctx, w, h, 0.9, "settle");
   drawLabel(ctx, "Server Scout!", w * 0.5, layout.labelY);
   failFlash(ctx, w, h); successFlash(ctx, w, h);
  });
  setDispose(() => {});
 });
