/**
 * Backend Builder boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
try {
 ...
 await initI18n({ localeStorageKey: manifest.localeKey || "gq-backend-builder-locale" });
 ...
 if (coachName) coachName.textContent = manifest.coachName || "Coach Server";
 ...
 const storageKey = manifest.storageKey || "gq-backend-builder-save-v2";

 if (!canvas) throw new Error("Missing #c3d canvas");

 const arena = createArena2D(canvas, { defaultScene: "serverMeet" });
 window.__arena = arena;
 registerServerScenes(arena);
 registerRoutesScenes(arena);
 registerAuthScenes(arena);
 ...
 const legacy = !saved ? loadSave("gq-backend-builder-save-v1") : null;
 ...
 gameTitle: manifest.title || "Backend Builder",
 missions: MISSIONS,
 completed: state.completed,
 forceAllLocked: false,
 unlockByProgress: false,
 subtitle: "Missions 1-3 are live (Canvas 2D). Server Basics - Routes & APIs - Auth Lite.",
 ...
 const introScene =
 state.level === 0
 ? ["serverMeet", { phase: "desk", dwellMs: 3200 }]
 : state.level === 1
 ? ["routesMeet", { phase: "desk", dwellMs: 3200 }]
 : state.level === 2
 ? ["authMeet", { phase: "desk", dwellMs: 3200 }]
 : ["serverMeet", { phase: "settle", dwellMs: 2400 }];
 ...
 showNext(false);
 clearOverlay();
 const winScene = state.level === 0 ? "serverMastery" : state.level === 1 ? "routesMastery" : state.level === 2 ? "authMastery" : "serverMastery";
 playScene(winScene);
 if (overlay) {
 overlay.innerHTML = `
 <div class="chem-card">
 ...
 playScene("serverMeet", { phase: "settle" });
 overlay.innerHTML = `
 <div class="chem-card">
 ...
 "Dial until more endpoint doors open.",
 "Sort route paths vs methods/status vs not.",
 "Open even more doors on the dial.",
 "Order path -> job -> method -> 404.",
 "Build: Path - Job - Method - Door.",
 "Tap home / school / shop / bd / lab paths.",
 "Bust myths about 404 and GET/POST.",
 "Hit the routes fluency checks.",
 "Claim Route Ranger.",
 ],
 [
 "Drag ID card and lock - prove who you are.",
 "Dial until the private room unlocks.",
 "Sort auth vs public vs not auth.",
 "Push the identity check stronger.",
 "Order ask -> proof -> token -> enter.",
 "Build: Prove - who - then - enter.",
 "Tap home / school / bank / bd / lab logins.",
 "Bust myths about sharing passwords.",
 "Hit the auth fluency checks.",
 "Claim Auth Guard.",
 ],
 ];
 ...
 if (!confirm("Reset all Backend Builder progress?")) return;
 clearSave(storageKey);
 clearSave("gq-backend-builder-save-v1");
 state.level = 0;
 state.sub = 0;
 ...
 // QA jump helper (tools/_qa-jump-backend.html) may set sessionStorage to resume mid-mission.
