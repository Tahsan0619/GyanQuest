/**
 * Math Quest activity mounts - panel + canvas share labState / intents.
 */
...
export function mountHeatLab(host, cfg) {
 ...
 // Defaults are place-value friendly; callers override for other missions (e.g. fractions).
 const sliderLabel = cfg.sliderLabel || "Build value";
 ...
 ${badgeHtml(cfg.badge || ATOM_ASSET_PATHS.m1, "place lab")}
 <h3>${cfg.title}</h3>
 <p>${cfg.html}</p>
...
export function mountEquationBuild(host, cfg) {
 ...
 if (ok) {
 status.textContent = "Rule locked in. Continue when ready.";
...
export function mountScaleLab(host, cfg) {
 ...
 // Defaults are place-value friendly; callers can override for other missions.
 const sliderLabel = cfg.sliderLabel || "Place scale: ones → ten-rod → chart";
 const goalText = cfg.goalText || "Left canvas follows: ones → ten → named place.";
...
export function mountSpeedDrill(host, cfg) {
 ...
 playScene(cfg.passScene || cfg.scene || "numMastery");
 host.innerHTML = `
 <div class="chem-card">
 ...
 if (it.tens != null) chemLabState.tens = it.tens;
 if (it.ones != null) chemLabState.ones = it.ones;
 playScene(cfg.scene || "numDrill", {
 prompt: chemLabState.prompt,
 flashColor: chemLabState.flashColor,
 tens: chemLabState.tens,
 ones: chemLabState.ones,
 });
 host.innerHTML = `
...
export function mountMythCards(host, cfg) {
 ...
 playScene(cfg.scene || "numMyth", { myth: m.sceneMyth });
 host.innerHTML = `
