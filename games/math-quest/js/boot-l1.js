/**
 * Math Quest boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
try {
 ...
 await initI18n({ localeStorageKey: manifest.localeKey || "gq-math-quest-locale" });
 ...
 if (coachName) coachName.textContent = manifest.coachName || "Coach Math";
 ...
 const storageKey = manifest.storageKey || "gq-math-quest-save-v2";
 ...
 const legacy = !saved ? loadSave("gq-math-quest-save-v1") : null;
 ...
 gameTitle: manifest.title || "Math Quest",
 missions: MISSIONS,
 completed: state.completed,
 forceAllLocked: false,
 unlockByProgress: false,
 subtitle: "Missions 1-3 are live (Canvas 2D). Number Sense · Fraction Friends.",
 ...
 if (!confirm("Reset all Math Quest progress?")) return;
 clearSave(storageKey);
 clearSave("gq-math-quest-save-v1");
 state.level = 0;
 state.sub = 0;
 ...
 document.documentElement.classList.add("mathquest-live", "chem-l1-live");

 // QA jump helper (tools/_qa-jump-math.html) may set sessionStorage to resume mid-mission.
