...
export function registerNumScenes(arena) {
  ...
  arena.registerScene("numRule", (api) => {
    ...
    setTick(() => {
      ...
      drawRod(ctx, w * 0.35, h * 0.58, 0.7);
      for (let i = 0; i < 10; i++) drawCube(ctx, w * 0.55 + (i % 5) * 22, h * 0.52 + Math.floor(i / 5) * 24, 0.75);
      drawLabel(ctx, "Number Sense rule: tens & ones", w * 0.5, layout.labelY);
      failFlash(ctx, w, h);
      successFlash(ctx, w, h);
    });
    setDispose(() => {});
  ...
  arena.registerScene("numStretch", (api) => {
    ...
    setTick(() => {
      ...
      drawPlaceChart(ctx, w * 0.5, h * 0.36, 2, 5);
      const captions = {
        eggs: "25 eggs = 2 bundles of 10 + 5 ones",
        taka: "25 taka = two 10s + a 5",
        cricket: "25 runs - think tens and ones on the scoreboard",
        bus: "25 seats filled - groups of ten help count fast",
        beads: "25 beads on a string - decade marks every 10",
      };
  ...
  arena.registerScene("numDrill", (api) => {
    ...
    setTick(() => {
      ...
      const h = api.height;
      drawBackdrop();
      drawLabel(ctx, labState.prompt || "Number Sense drill", w * 0.5, h * 0.2, {
        h: 32,
        font: "700 16px Segoe UI",
      });
      drawPlaceChart(ctx, w * 0.5, h * 0.48, labState.tens ?? 3, labState.ones ?? 1);
      failFlash(ctx, w, h);
      successFlash(ctx, w, h);
    });
    setDispose(() => {});
  ...
  arena.registerScene("numMastery", (api) => {
    ...
    setTick(() => {
      ...
      drawRod(ctx, w * 0.4, h * 0.4, 0.9);
      drawCube(ctx, w * 0.58, h * 0.48, 1.2);
      drawLabel(ctx, "Number Scout!", w * 0.5, layout.labelY);
      failFlash(ctx, w, h);
      successFlash(ctx, w, h);
    });
    setDispose(() => {});
