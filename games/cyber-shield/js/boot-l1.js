/**
 * Cyber Shield boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { runL1Sub, L1_META } from "./level1.js?v=tier3";
import { runL2Sub, L2_META } from "./level2.js?v=tier3";
import { runL3Sub, L3_META } from "./level3.js?v=tier3";
...
import { registerPassScenes } from "./pass-scenes.js";
...
export async function bootCyberShield({ manifest }) {

const BOOKS = [BOOK_L1];
let bookApi = { onBookClick() { console.warn("[books] unavailable"); } };
try {
 bookApi = setupMissionBooks({
 subject: manifest.title || "cyber-shield",
 ...
 await initI18n({ localeStorageKey: manifest.localeKey || "gq-cyber-shield-locale" });
 ...
 if (coachName) coachName.textContent = manifest.coachName || "Coach Shield";
 ...
 const storageKey = manifest.storageKey || "gq-cyber-shield-save-v2";

 if (!canvas) throw new Error("Missing #c3d canvas");

 const arena = createArena2D(canvas, { defaultScene: "passMeet" });
 window.__arena = arena;
 registerPassScenes(arena);
 ...
 const legacy = !saved ? loadSave("gq-cyber-shield-save-v1") : null;
 ...
 gameTitle: manifest.title || "Cyber Shield",
 missions: MISSIONS,
 completed: state.completed,
 forceAllLocked: false,
 unlockByProgress: false,
 subtitle: "Mission 1 is live (Canvas 2D). Password Power.",
 ...
 const introScene =
 state.level === 0
 ? ["passMeet", { phase: "desk", dwellMs: 3200 }]
 : ["passMeet", { phase: "settle", dwellMs: 2400 }];
 ...
 playScene("passMeet", { phase: "settle" });
 overlay.innerHTML = `
 <div class="chem-card">
 ...
 if (!confirm("Reset all Cyber Shield progress?")) return;
 clearSave(storageKey);
 clearSave("gq-cyber-shield-save-v1");
 state.level = 0;
 state.sub = 0;
 ...
 document.documentElement.classList.add("cybershield-live", "chem-l1-live");
