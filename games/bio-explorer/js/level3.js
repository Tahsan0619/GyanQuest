...
export const L3_META = {
  ...
  bdHook: "Bangladesh everyday: notice plants around you — then connect it to Plant Power.",
  predict: {
    q: "Before we start — what do you think matters most in Plant Power?",
    options: [
...
function sub2_sun({ overlay, setCoach, completeSub }) {
  ...
  mountHeatLab(overlay, {
    ...
    html: `<p>Drag the sun handle - watch leaves, flower, then fruit.</p>`,
...
function sub4_grow({ overlay, setCoach, completeSub }) {
  setCoach("Push sun high - seed -> leaf -> flower -> fruit.");
  bioLabState.heat = 0.4;
  mountHeatLab(overlay, {
    scene: "plantLab",
    title: "Grow Stages",
    html: `<p>Reach 80% sun energy to see full plant stages.</p>`,
...
function sub7_stretch({ overlay, setCoach, completeSub }) {
  ...
  mountTapContinue(overlay, {
    scene: "plantStretch",
    html: `<h3>Bangladesh stories</h3><p>Tap mango, rice, rose, bamboo, algae.</p>`,
    onDone: () => {
      mountQuiz(overlay, {
        scene: "plantStretch",
        title: "Transfer",
