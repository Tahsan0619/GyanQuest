/**
 * Bio Explorer boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerLifeScenes } from "./life-scenes.js";
import { registerCellScenes } from "./cell-scenes.js";
import { registerPlantScenes } from "./plant-scenes.js";
...
try {
  ...
  await initI18n({ localeStorageKey: manifest.localeKey || "gq-bio-explorer-locale" });
  ...
  if (coachName) coachName.textContent = manifest.coachName || "Coach Bio";
  ...
  const storageKey = manifest.storageKey || "gq-bio-explorer-save-v2";
  ...
  const legacy = !saved ? loadSave("gq-bio-explorer-save-v1") : null;
  ...
  function showHub() {
    ...
    mountMissionHub(hubRoot, {
      ...
      gameTitle: manifest.title || "Bio Explorer",
      missions: MISSIONS,
      completed: state.completed,
      forceAllLocked: false,
      unlockByProgress: false,
      subtitle: "Missions 1–3 are live (Canvas 2D). Living or Not → Cell City → Plant Power.",
  ...
  function runComingSoon() {
    ...
    playScene("lifeMeet", { phase: "settle" });
    overlay.innerHTML = `
      <div class="chem-card">
  ...
  btnResetAll?.addEventListener("click", () => {
    if (!confirm("Reset all Bio Explorer progress?")) return;
    clearSave(storageKey);
    clearSave("gq-bio-explorer-save-v1");
    state.level = 0;
    state.sub = 0;
  ...
  document.documentElement.classList.add("bio-l1-live", "chem-l1-live");
