/**
 * Discrete Math & Logic boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerLogicScenes } from "./logic-scenes.js";
...
export async function bootDiscreteMath({ manifest }) {

const BOOKS = [BOOK_L1];
let bookApi = { onBookClick() { console.warn("[books] unavailable"); } };
try {
 bookApi = setupMissionBooks({
 subject: manifest.title || "discrete-math",
 ...
 await initI18n({ localeStorageKey: manifest.localeKey || "gq-discrete-math-locale" });
 ...
 if (coachName) coachName.textContent = manifest.coachName || "Coach Logic";
 ...
 const storageKey = manifest.storageKey || "gq-discrete-math-save-v2";

 if (!canvas) throw new Error("Missing #c3d canvas");

 const arena = createArena2D(canvas, { defaultScene: "logicMeet" });
 window.__arena = arena;
 registerLogicScenes(arena);
 ...
 const legacy = !saved ? loadSave("gq-discrete-math-save-v1") : null;
 ...
 gameTitle: manifest.title || "Discrete Math & Logic",
 missions: MISSIONS,
 completed: state.completed,
 forceAllLocked: false,
 unlockByProgress: false,
 subtitle: "Mission 1 is live (Canvas 2D). Logic Lite.",
 ...
 const introScene =
 state.level === 0
 ? ["logicMeet", { phase: "desk", dwellMs: 3200 }]
 : state.level === 1
 ? ["pushMeet", { phase: "predict", dwellMs: 3200 }]
 : state.level === 2
 ? ["pairMeet", { phase: "desk", dwellMs: 3200 }]
 : ["logicMeet", { phase: "settle", dwellMs: 2400 }];
 ...
 playScene("logicMeet", { phase: "settle" });
 overlay.innerHTML = `
 <div class="chem-card">
 ...
 const hints = [
 [
 "Meet AND, OR, NOT gates.",
 "Dial until logic idea is clear.",
 "Sort logic pieces vs not-logic.",
 "Push gate decision clarity.",
 "Order inputs -> gate -> rule -> true/false.",
 "Build: AND OR NOT decide.",
 "Tap shoes (AND), ride (OR), rain (NOT).",
 "Bust myths that AND equals OR.",
 "Hit logic fluency checks.",
 "Claim Logic Learner.",
 ],
 [
 "Coming soon hint.",
 ...
 if (!confirm("Reset all Discrete Math & Logic progress?")) return;
 clearSave(storageKey);
 clearSave("gq-discrete-math-save-v1");
 state.level = 0;
 state.sub = 0;
 ...
 document.documentElement.classList.add("discretemath-live", "chem-l1-live");
