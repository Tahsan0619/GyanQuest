...
export function mountHeatLab(host, cfg) {
 ...
 // Defaults keep Strong Structures framing; Mission labs pass topic overrides.
 const sliderLabel = cfg.sliderLabel || "Strength (braces + base)";
 ...
 ${badgeHtml(cfg.badge || ATOM_ASSET_PATHS.m1, "structure lab")}
 <h3>${cfg.title}</h3>
 <p>${cfg.html}</p>
...
export function mountScaleLab(host, cfg) {
 ...
 // Defaults keep Strong Structures framing; Mission 1 passes topic overrides.
 const sliderLabel = cfg.sliderLabel || "Structure scale: desk \u2192 members \u2192 STRUCTURE";
 const goalText = cfg.goalText || "Left canvas follows: everyday frames \u2192 load path \u2192 STRUCTURE banner.";
...
export function mountSpeedDrill(host, cfg) {
 ...
 playScene(cfg.passScene || cfg.scene || "structMastery");
 host.innerHTML = `
 <div class="chem-card">
 ...
 playScene(cfg.scene || "structDrill", { prompt: chemLabState.prompt, flashColor: chemLabState.flashColor });
 host.innerHTML = `
...
export function mountMythCards(host, cfg) {
 ...
 playScene(cfg.scene || "structMyth", { myth: m.sceneMyth });
 host.innerHTML = `
...
export function mountSpiralMap(host, cfg) {
 ...
 playScene(cfg.scene || "structSpiral");
 ...
 <button type="button" class="btn primary tiny-pulse" id="spiral-go">${cfg.finishLabel || "Finish Strong Structures ▶"}</button>
 </div>`;
 function playStop(n) {
 if (cancelled) return;
 chemLabState.spiralStop = n;
 chemLabState.spiralUntil = performance.now() + 4500;
 playScene(cfg.scene || "structSpiral", { spiralStop: n });
 }
