/**
 * Civil Basics boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { playScene, cancelActiveActivity } from "./lab-activities.js?v=struct3";
import { runL1Sub, L1_META } from "./level1.js?v=struct3";
...
import { registerStructScenes } from "./struct-scenes.js?v=struct3";
import { BOOK as BOOK_L1 } from "../books/level1.js?v=gencv1";
...
export async function bootCivilBasics({ manifest }) {

const BOOKS = [BOOK_L1];
let bookApi = { onBookClick() { console.warn("[books] unavailable"); } };
try {
 bookApi = setupMissionBooks({
 subject: manifest.title || "civil-basics",
 ...
 await initI18n({ localeStorageKey: manifest.localeKey || "gq-civil-basics-locale" });
 ...
 if (coachName) coachName.textContent = manifest.coachName || "Coach Beam";
 ...
 const storageKey = manifest.storageKey || "gq-civil-basics-save-v2";

 if (!canvas) throw new Error("Missing #c3d canvas");

 const arena = createArena2D(canvas, { defaultScene: "structOpen" });
 window.__arena = arena;
 registerStructScenes(arena);
 ...
 const legacy = !saved ? loadSave("gq-civil-basics-save-v1") : null;
 ...
 gameTitle: manifest.title || "Civil Basics",
 missions: MISSIONS,
 completed: state.completed,
 forceAllLocked: false,
 unlockByProgress: false,
 subtitle: "Mission 1: Strong Structures (interactive Bruner lesson). More missions coming soon.",
 ...
 ? ["structOpen", {}]
 : state.level === 1
 ? ["structMeet", { phase: "desk", dwellMs: 3200 }]
 : state.level === 2
 ? ["structMeet", { phase: "desk", dwellMs: 3200 }]
 : ["structOpen", {}];
 ...
 playScene("structMeet", { phase: "settle" });
 overlay.innerHTML = `
 <div class="chem-card">
 ...
 const hints = [
 [
 "Compare a triangle frame to a tall skinny tower.",
 "Dial strength until the bridge holds the load.",
 "Sort strong, weak, and not-structure ideas.",
 "Push strength higher with braces and base.",
 "Order why the bridge holds.",
 "Build: Triangles + wide base carry load.",
 "Tap home / school / street / shop / lab.",
 "Bust structure myths.",
 "Hit the fluency checks.",
 "Claim Structure Scout.",
 ],
 ];
 ...
 if (!confirm("Reset all Civil Basics progress?")) return;
 clearSave(storageKey);
 clearSave("gq-civil-basics-save-v1");
 state.level = 0;
 state.sub = 0;
 ...
 document.documentElement.classList.add("civilbasics-live", "chem-l1-live");
