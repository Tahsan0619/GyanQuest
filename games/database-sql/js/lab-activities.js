/**
 * Database & SQL activity mounts - panel + canvas share labState / intents.
 */
...
export function mountHeatLab(host, cfg) {
 ...
 const sliderLabel = cfg.sliderLabel || "Row fill";
 ...
 ${badgeHtml(cfg.badge || ATOM_ASSET_PATHS.m1, "table lab")}
 <h3>${cfg.title}</h3>
 <p>${cfg.html}</p>
...
export function mountScaleLab(host, cfg) {
 ...
 // Table-scale defaults (override via cfg for other missions in this game).
 const sliderLabel = cfg.sliderLabel || "Table scale: sheet \u2192 columns \u2192 rows \u2192 cell";
 const goalText = cfg.goalText || "Left canvas: blank sheet \u2192 headers \u2192 rows \u2192 cell/record.";
...
export function mountSpeedDrill(host, cfg) {
 ...
 playScene(cfg.passScene || cfg.scene || "tableMastery");
 host.innerHTML = `
 <div class="chem-card">
 ...
 playScene(cfg.scene || "tableDrill", { prompt: chemLabState.prompt, flashColor: chemLabState.flashColor });
 host.innerHTML = `
...
export function mountMythCards(host, cfg) {
 ...
 playScene(cfg.scene || "tableMyth", { myth: m.sceneMyth });
 host.innerHTML = `
