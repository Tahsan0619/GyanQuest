/**
 * Database & SQL boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
try {
 ...
 await initI18n({ localeStorageKey: manifest.localeKey || "gq-database-sql-locale" });
 ...
 if (coachName) coachName.textContent = manifest.coachName || "Coach Query";
 ...
 const storageKey = manifest.storageKey || "gq-database-sql-save-v2";

 if (!canvas) throw new Error("Missing #c3d canvas");

 const arena = createArena2D(canvas, { defaultScene: "tableMeet" });
 window.__arena = arena;
 registerTableScenes(arena);
 registerSelectScenes(arena);
 registerJoinScenes(arena);
 ...
 const legacy = !saved ? loadSave("gq-database-sql-save-v1") : null;
 ...
 gameTitle: manifest.title || "Database & SQL",
 missions: MISSIONS,
 completed: state.completed,
 forceAllLocked: false,
 unlockByProgress: false,
 subtitle: "Missions 1-3 are live (Canvas 2D). Tables & Rows - SELECT Stories - Keys & Joins.",
 ...
 const introScene =
 state.level === 0
 ? ["tableMeet", { phase: "desk", dwellMs: 3200 }]
 : state.level === 1
 ? ["selectMeet", { phase: "desk", dwellMs: 3200 }]
 : state.level === 2
 ? ["joinMeet", { phase: "desk", dwellMs: 3200 }]
 : ["tableMeet", { phase: "settle", dwellMs: 2400 }];
 ...
 <p class="chem-intro__brunner"><strong>Bruner path:</strong> do and see, then pictures, then name the rule.</p>`
 ...
 showNext(false);
 clearOverlay();
 const winScene = state.level === 0 ? "tableMastery" : state.level === 1 ? "selectMastery" : state.level === 2 ? "joinMastery" : "tableMastery";
 playScene(winScene);
 if (overlay) {
 overlay.innerHTML = `
 <div class="chem-card">
 ...
 playScene("tableMeet", { phase: "settle" });
 overlay.innerHTML = `
 <div class="chem-card">
 ...
 "Dial until WHERE filters clearly.",
 "Sort ask/SELECT vs write vs not SQL.",
 "Sharpen the query on the dial.",
 "Order SELECT -> FROM -> WHERE -> rows.",
 "Build: SELECT - FROM - WHERE - rows.",
 "Tap home / school / shop / bd / lab questions.",
 "Bust myths about SELECT rewriting tables.",
 "Hit the SELECT fluency checks.",
 "Claim Query Kid.",
 ],
 [
 "See two tables - matching keys draw a link.",
 "Dial until the key match is strong.",
 "Sort key fields vs join actions vs not.",
 "Strengthen the join link on the dial.",
 "Order key -> store -> match -> story.",
 "Build: Key - Match - Join - Link.",
 "Tap home / school / shop / bd / lab links.",
 "Bust myths about glue and orphan rows.",
 "Hit the join fluency checks.",
 "Claim Join Junior.",
 ],
 ];
 ...
 if (!confirm("Reset all Database & SQL progress?")) return;
 clearSave(storageKey);
 clearSave("gq-database-sql-save-v1");
 state.level = 0;
 state.sub = 0;
 ...
 document.documentElement.classList.add("databasesql-live", "chem-l1-live");

 // QA jump helper (tools/_qa-jump-db.html) may set sessionStorage to resume mid-mission.
 const qaJump = typeof sessionStorage !== "undefined" && sessionStorage.getItem("gq-qa-jump") === "1";
 if (qaJump) {
 try { sessionStorage.removeItem("gq-qa-jump"); } catch (_) {}
 resumePlaySession();
 } else {
