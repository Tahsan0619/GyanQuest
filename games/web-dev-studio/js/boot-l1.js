/**
 * Web Dev Studio boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerHtmlScenes } from "./html-scenes.js";
import { registerCssScenes } from "./css-scenes.js";
import { registerJsScenes } from "./js-scenes.js";
...
try {
  ...
  await initI18n({ localeStorageKey: manifest.localeKey || "gq-web-dev-studio-locale" });
  ...
  if (coachName) coachName.textContent = manifest.coachName || "Coach Web";
  ...
  const storageKey = manifest.storageKey || "gq-web-dev-studio-save-v2";

  if (!canvas) throw new Error("Missing #c3d canvas");

  const arena = createArena2D(canvas, { defaultScene: "htmlMeet" });
  window.__arena = arena;
    registerHtmlScenes(arena);
  registerCssScenes(arena);
  registerJsScenes(arena);
  ...
  const legacy = !saved ? loadSave("gq-web-dev-studio-save-v1") : null;
  ...
  function showHub() {
    ...
    mountMissionHub(hubRoot, {
      ...
      gameTitle: manifest.title || "Web Dev Studio",
      missions: MISSIONS,
      completed: state.completed,
      forceAllLocked: false,
      unlockByProgress: false,
      subtitle: "Missions 1–3 are live (Canvas 2D). HTML House · CSS Style · JS Click.",
  ...
  function openIntro(then) {
    ...
    const introScene =
      state.level === 0
        ? ["htmlMeet", { phase: "desk", dwellMs: 3200 }]
        : state.level === 1
          ? ["cssMeet", { phase: "desk", dwellMs: 3200 }]
          : state.level === 2
            ? ["jsMeet", { phase: "desk", dwellMs: 3200 }]
            : ["htmlMeet", { phase: "settle", dwellMs: 2400 }];
  ...
  function finishMission() {
    ...
    clearOverlay();
    const winScene =
      state.level === 0 ? "htmlMastery" : state.level === 1 ? "cssMastery" : state.level === 2 ? "jsMastery" : "htmlMastery";
    playScene(winScene);
    if (overlay) {
      overlay.innerHTML = `
        <div class="chem-card">
  ...
  function runComingSoon() {
    ...
    playScene("htmlMeet", { phase: "settle" });
    overlay.innerHTML = `
      <div class="chem-card">
  ...
  btnResetAll?.addEventListener("click", () => {
    if (!confirm("Reset all Web Dev Studio progress?")) return;
    clearSave(storageKey);
    clearSave("gq-web-dev-studio-save-v1");
    state.level = 0;
    state.sub = 0;
  ...
  document.documentElement.classList.add("webdevstudio-live", "chem-l1-live");
