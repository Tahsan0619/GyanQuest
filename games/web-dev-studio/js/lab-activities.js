...
export function mountHeatLab(host, cfg) {
 ...
 // Defaults keep web-dev framing; HTML / CSS / JS labs pass topic overrides.
 const sliderLabel = cfg.sliderLabel || "Build / style / click level";
 ...
 ${badgeHtml(cfg.badge || ATOM_ASSET_PATHS.m1, "web lab")}
 <h3>${cfg.title}</h3>
 <p>${cfg.html}</p>
 ...
 <p class="chem-heat__readout" id="chem-heat-read" aria-live="polite">Starting…</p>
 ...
 cold: "Quiet page - keep building",
 melting: "Opening up - tags, style, or clicks rising",
 liquid: "Clearer - almost at the goal",
 simmer: "Strong - page feels ready",
 };
...
export function mountScaleLab(host, cfg) {
 ...
 // Defaults keep web-dev framing; missions may pass topic overrides.
 const sliderLabel = cfg.sliderLabel || "Web scale: page -> style/detail -> rule";
 const goalText = cfg.goalText || "Left canvas follows: page parts -> closer look -> rule banner.";
 const readouts = cfg.readoutLabels || {
 low: "Page view - HTML rooms together",
 mid: "Closer detail - CSS look or click path",
...
export function mountSpeedDrill(host, cfg) {
 ...
 playScene(cfg.passScene || cfg.scene || "htmlMastery");
 host.innerHTML = `
 <div class="chem-card">
 ...
 playScene(cfg.scene || "htmlDrill", { prompt: chemLabState.prompt, flashColor: chemLabState.flashColor });
 host.innerHTML = `
...
export function mountMythCards(host, cfg) {
 ...
 playScene(cfg.scene || "htmlMyth", { myth: m.sceneMyth });
 host.innerHTML = `
...
export function mountGate(host, cfg) {
 ...
 iv = cfg.ready || cfg.liveStatus
 ? setInterval(() => {
 if (cancelled) return;
 if (cfg.liveStatus && status && !(cfg.ready && cfg.ready())) {
 const live = cfg.liveStatus();
 if (live) status.textContent = live;
 }
 ...
 if (cfg.bind) bindCleanup = cfg.bind(host, { finish, button: btn, status, playScene });
 btn.onclick = () => {
 if (btn.disabled) return;
 finish();
 };
 ...
 if (typeof bindCleanup === "function") bindCleanup();
...
export function mountSpiralMap(host, cfg) {
 ...
 playScene(cfg.scene || "htmlSpiral");
 ...
 <button type="button" class="btn primary tiny-pulse" id="spiral-go">${cfg.finishLabel || "Finish HTML House ▶"}</button>
 </div>`;
 function playStop(n) {
 if (cancelled) return;
 chemLabState.spiralStop = n;
 chemLabState.spiralUntil = performance.now() + 4500;
 playScene(cfg.scene || "htmlSpiral", { spiralStop: n });
 }
