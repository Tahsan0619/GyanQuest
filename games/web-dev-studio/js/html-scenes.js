...
export function registerHtmlScenes(arena) {
 ...
 function drawModePage(mode, cx, cy, scale) {
 const bw = 200 * scale, bh = 120 * scale;
 drawBrowserChrome(ctx, cx - bw / 2, cy - bh / 2 - 10, bw, bh + 10, mode + ".html");
 const ix = cx - bw / 2 + 12, iy = cy - bh / 2 + 18;
 if (mode === "home") {
 ctx.fillStyle = "#fdba74"; roundRect(ctx, ix, iy, 50, 40, 4); ctx.fill();
 ctx.fillStyle = "#ffedd5"; ctx.font = "700 11px Segoe UI"; ctx.textAlign = "left";
 ctx.fillText("<h1> Family", ix + 60, iy + 14);
 ctx.fillText("<p> Hello!", ix + 60, iy + 32);
 } else if (mode === "school") {
 ctx.fillStyle = "#ea580c"; ctx.font = "700 12px Segoe UI"; ctx.textAlign = "left";
 ctx.fillText("NOTICE", ix, iy + 12);
 ctx.fillStyle = "#94a3b8"; ctx.font = "600 10px Segoe UI";
 ctx.fillText("<h1> Assembly 9am", ix, iy + 32);
 ctx.fillText("<p> Bring books", ix, iy + 48);
 ctx.fillText("<p> Gate open", ix, iy + 64);
 } else if (mode === "shop") {
 for (let i = 0; i < 3; i++) {
 ctx.fillStyle = i === 1 ? "#fb923c" : "#334155";
 roundRect(ctx, ix + i * 58, iy + 8, 50, 56, 6); ctx.fill();
 ctx.fillStyle = "#ffedd5"; ctx.font = "600 9px Consolas, monospace"; ctx.textAlign = "center";
 ctx.fillText("<div>", ix + i * 58 + 25, iy + 38);
 }
 } else if (mode === "bd") {
 ctx.fillStyle = "#c2410c"; roundRect(ctx, ix, iy, bw - 24, 28, 4); ctx.fill();
 ctx.fillStyle = "#fff7ed"; ctx.font = "700 11px Segoe UI"; ctx.textAlign = "left";
 ctx.fillText("<h1> BD headlines", ix + 8, iy + 18);
 ctx.fillStyle = "#94a3b8"; ctx.font = "600 10px Segoe UI";
 ctx.fillText("<p> Nested story blocks…", ix, iy + 48);
 } else {
 ctx.fillStyle = "#ea580c"; ctx.font = "700 13px Segoe UI"; ctx.textAlign = "left";
 ctx.fillText("<h1> My trip", ix, iy + 14);
 ctx.fillStyle = "#64748b"; ctx.font = "600 10px Segoe UI";
 ctx.fillText("<p> Story paragraph…", ix, iy + 36);
 ctx.fillStyle = "#fdba74"; roundRect(ctx, ix, iy + 48, 70, 36, 4); ctx.fill();
 ctx.fillStyle = "#7c2d12"; ctx.font = "700 9px Consolas"; ctx.textAlign = "center";
 ctx.fillText("<img>", ix + 35, iy + 68);
 }
 }
 setTick(() => {
 ...
 ctx.fillStyle = m.id === mode ? "rgba(251,146,60,0.45)" : "#1e293b";
 roundRect(ctx, x - 36, layout.deskTop - 36, 72, 48, 10); ctx.fill();
 ctx.fillStyle = "#ffedd5"; ctx.font = "600 11px Segoe UI"; ctx.textAlign = "center"; ctx.fillText(m.label, x, layout.deskTop - 10);
 hits.push({ id: m.id, shape: "rect", x, y: layout.deskTop - 12, w: 72, h: 48, meta: { mode: m.id } });
 });
 drawModePage(mode, w * 0.5, h * 0.4, Math.min(w, h) / 420);
 drawHtmlHouse(ctx, w * 0.82, h * 0.55, Math.min(w, h) / 700, 0.7, "settle");
 const captions = {
 home: "Family homepage - still <html><body> rooms",
 school: "School notice - headings + paragraphs",
 ...
 const { ctx, setTick, setDispose, setDescription, drawBackdrop, setHitRegions, setIntentHandler } = api;
 const myths = [
 { claim: "HTML is only for experts", truth: "Kids can build page rooms with a few clear tags", kind: "experts" },
 { claim: "Tags can stay open forever", truth: "Most tags need a matching close tag", kind: "open" },
 { claim: "Order of tags never matters", truth: "Nesting order builds the page house", kind: "order" },
 { claim: "CSS and HTML are the same", truth: "HTML = structure; CSS = look", kind: "same" },
 { claim: "One giant <div> is enough forever", truth: "Clear tags (h1, p, img) help people and browsers", kind: "div" },
 ];
 ...
 if (phase === "claim") {
 if (kind === "experts") {
 ctx.fillStyle = "#334155"; roundRect(ctx, cx - 50, cy - 40, 100, 70, 8); ctx.fill();
 ctx.fillStyle = "#f87171"; ctx.font = "800 22px Segoe UI"; ctx.textAlign = "center";
 ctx.fillText("🔒", cx, cy - 5);
 ctx.fillStyle = "#fca5a5"; ctx.font = "600 11px Segoe UI"; ctx.fillText("Experts only?", cx, cy + 28);
 } else if (kind === "open") {
 ctx.fillStyle = "#f87171"; ctx.font = "700 14px Consolas"; ctx.textAlign = "center";
 ctx.fillText("<h1> <p> <div>", cx, cy - 10);
 ctx.fillText("(no closes…)", cx, cy + 16);
 } else if (kind === "order") {
 ctx.fillStyle = "#f87171"; ctx.font = "700 12px Consolas"; ctx.textAlign = "center";
 ctx.fillText("</body> <html> <p>", cx, cy);
 ctx.fillText("scrambled soup", cx, cy + 22);
 } else if (kind === "same") {
 ...
 ctx.fillText("HTML = CSS?", cx, cy + 4);
 } else {
 ctx.fillStyle = "#7f1d1d"; roundRect(ctx, cx - 70, cy - 35, 140, 70, 8); ctx.fill();
 ctx.fillStyle = "#fecaca"; ctx.font = "700 14px Consolas"; ctx.textAlign = "center";
 ctx.fillText("<div>…everything…", cx, cy);
 }
 return;
 }
 if (kind === "experts") {
 ["<html>", "<h1>", "<p>"].forEach((t, i) => {
 ctx.fillStyle = "#ea580c"; roundRect(ctx, cx - 70 + i * 50, cy - 20, 44, 28, 6); ctx.fill();
 ctx.fillStyle = "#fff7ed"; ctx.font = "700 10px Consolas"; ctx.textAlign = "center";
 ctx.fillText(t, cx - 48 + i * 50, cy - 4);
 });
 } else if (kind === "open") {
 ctx.fillStyle = "#22c55e"; ctx.font = "700 13px Consolas"; ctx.textAlign = "center";
 ctx.fillText("<h1>…</h1>", cx, cy - 12);
 ctx.fillText("<p>…</p>", cx, cy + 14);
 } else if (kind === "order") {
 drawHtmlHouse(ctx, cx, cy + 10, 0.55, 0.85, "settle");
 } else if (kind === "same") {
 ...
 ctx.fillText("HTML", cx - 50, cy); ctx.fillText("structure", cx - 50, cy + 14);
 ctx.fillText("CSS", cx + 50, cy); ctx.fillText("look", cx + 50, cy + 14);
 } else {
 ["<h1>", "<p>", "<img>"].forEach((t, i) => {
 ctx.fillStyle = "#fb923c"; roundRect(ctx, cx - 80 + i * 55, cy - 18, 50, 36, 6); ctx.fill();
 ctx.fillStyle = "#0f172a"; ctx.font = "700 11px Consolas"; ctx.textAlign = "center";
 ctx.fillText(t, cx - 55 + i * 55, cy);
 });
 }
 }
 setTick(() => {
 ...
 const prompt = (labState.prompt || "HTML").toLowerCase();
 drawBackdrop();
 drawLabel(ctx, labState.prompt || "HTML House drill", w * 0.5, h * 0.14, { h: 32, font: "700 16px Segoe UI" });
 const cx = w * 0.5, cy = h * 0.48;
 if (prompt.includes("body")) {
 drawBrowserChrome(ctx, cx - 90, cy - 55, 180, 110, "body.html");
 ctx.fillStyle = "#ea580c"; roundRect(ctx, cx - 70, cy - 20, 140, 50, 8); ctx.fill();
 ctx.fillStyle = "#fff7ed"; ctx.font = "700 14px Consolas"; ctx.textAlign = "center";
 ctx.fillText("<body> visible</body>", cx, cy + 5);
 } else if (prompt.includes("h1")) {
 ctx.fillStyle = "#ea580c"; roundRect(ctx, cx - 60, cy - 24, 120, 48, 8); ctx.fill();
 ctx.fillStyle = "#fff"; ctx.font = "800 18px Segoe UI"; ctx.textAlign = "center";
 ctx.fillText("<h1>", cx, cy);
 } else if (prompt.includes("style")) {
 ctx.fillStyle = "#38bdf8"; roundRect(ctx, cx - 70, cy - 24, 140, 48, 8); ctx.fill();
 ctx.fillStyle = "#0f172a"; ctx.font = "700 14px Consolas"; ctx.textAlign = "center";
 ctx.fillText("color:red", cx, cy);
 } else if (prompt.includes("nest")) {
 drawHtmlHouse(ctx, cx, cy, Math.min(w, h) / 480, 0.8, "settle");
 } else if (prompt.includes("cake")) {
 ctx.fillStyle = "#f472b6"; roundRect(ctx, cx - 50, cy - 30, 100, 60, 12); ctx.fill();
 ctx.fillStyle = "#831843"; ctx.font = "700 16px Segoe UI"; ctx.textAlign = "center";
 ctx.fillText("🎂 Cake", cx, cy); ctx.font = "600 11px Segoe UI";
 ctx.fillText("not a tag", cx, cy + 20);
 } else {
 drawHtmlHouse(ctx, cx, cy, Math.min(w, h) / 460, 0.75, "settle");
 }
 failFlash(ctx, w, h); successFlash(ctx, w, h);
 });
 setDispose(() => {});
 ...
 drawBrowserChrome(ctx, w * 0.12, h * 0.18, w * 0.42, h * 0.42, "built.html");
 drawHtmlHouse(ctx, w * 0.33, h * 0.4, Math.min(w, h) / 560, 0.9, "settle");
 ctx.fillStyle = "rgba(234,88,12,0.92)";
 roundRect(ctx, w * 0.58, h * 0.28, w * 0.32, h * 0.28, 14); ctx.fill();
 ctx.fillStyle = "#fff7ed"; ctx.font = "800 16px Segoe UI"; ctx.textAlign = "center";
 ctx.fillText("HTML Builder", w * 0.74, h * 0.4);
 ctx.font = "600 11px Segoe UI";
 ctx.fillText("Tags nest · close · house", w * 0.74, h * 0.48);
