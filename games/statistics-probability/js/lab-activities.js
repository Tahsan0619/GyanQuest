/**
 * Statistics & Probability activity mounts - panel + canvas share labState / intents.
 */
...
export function mountHeatLab(host, cfg) {
 ...
 // Defaults are Mean & Mode framing; callers override per lab (outlier / peak / chance).
 const sliderLabel = cfg.sliderLabel || "Data clarity";
 ...
 ${badgeHtml(cfg.badge || ATOM_ASSET_PATHS.m1, "data lab")}
 <h3>${cfg.title}</h3>
 <p>${cfg.html}</p>
 ...
 <p class="chem-heat__readout" id="chem-heat-read" aria-live="polite">Ready</p>
 ...
 melting: "Pattern emerging…",
 liquid: "Mean / mode story getting clear",
 simmer: "Average story locked!",
 };
...
export function mountEquationBuild(host, cfg) {
 ...
 if (ok) {
 status.textContent = cfg.successText || "Rule locked in. Continue when ready.";
...
export function mountScaleLab(host, cfg) {
 ...
 // Defaults are Mean & Mode framing; callers may override labels.
 const sliderLabel = cfg.sliderLabel || "Average scale: list → mean → mode";
 const goalText = cfg.goalText || "Left canvas follows: data list → sum÷count → mode peak.";
...
export function mountSpeedDrill(host, cfg) {
 ...
 playScene(cfg.passScene || cfg.scene || "meanMastery");
 host.innerHTML = `
 <div class="chem-card">
 ...
 playScene(cfg.scene || "meanDrill", { prompt: chemLabState.prompt, flashColor: chemLabState.flashColor });
 host.innerHTML = `
...
export function mountMythCards(host, cfg) {
 ...
 playScene(cfg.scene || "meanMyth", { myth: m.sceneMyth });
 host.innerHTML = `
...
export function mountSpiralMap(host, cfg) {
 ...
 playScene(cfg.scene || "meanSpiral");
 ...
 ${narrationHtml(cfg.narration || "Tap a spiral number to replay, then finish when ready.")}
 ...
 <button type="button" class="btn primary tiny-pulse" id="spiral-go">${cfg.finishLabel || "Finish Mean & Mode ▶"}</button>
 </div>`;
 function playStop(n) {
 if (cancelled) return;
 chemLabState.spiralStop = n;
 chemLabState.spiralUntil = performance.now() + 4500;
 playScene(cfg.scene || "meanSpiral", { spiralStop: n });
 }
