/**
 * Statistics & Probability Canvas 2D arena - layout-aware, hit-testable, resize-safe.
 */
...
export function createArena2D(canvas, theme = {}) {
 ...
 let lastSceneName = theme.defaultScene || "meanMeet";
 let hitRegions = [];
 let dragState = null;
 ...
 const description = text || "Statistics mean & mode stage";
 ...
 const rw = r.w || 40;
 const rh = r.h || 40;
 // Mean & Mode uses shape:"rect" with (x,y) as CENTER.
 // Other missions may omit shape and pass TOP-LEFT + size.
 ...
 clearScene();
 const fn = SCENES[name] || SCENES.meanMeet;
 if (fn) fn();
 ...
 playExample("meanMeet", { phase: "glow" });
 }
