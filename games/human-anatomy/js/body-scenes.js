/**
 * Body Map - Canvas 2D scenes (deepened).
 */
...
function drawLabel(ctx, text, x, y, opts = {}) {
  ...
  ctx.fill();
  ctx.strokeStyle = opts.border || "#fb7185";
  ctx.lineWidth = 1.4;
  ctx.stroke();
  ctx.fillStyle = opts.color || "#fecdd3";
...
function drawHero(ctx, w, h, heat, phase) {
  ...
  roundRect(ctx, cx - 50, cy - 70, 100, 140, 40); ctx.stroke();
  const organs = [
    { x: cx, y: cy - 40, r: 14, c: "#f9a8d4", t: "Brain" },
    { x: cx - 18, y: cy - 5, r: 12, c: "#7dd3fc", t: "Lung" },
    { x: cx + 18, y: cy - 5, r: 12, c: "#7dd3fc", t: "Lung" },
    { x: cx, y: cy + 18, r: 14, c: "#fb7185", t: "Heart" },
    { x: cx, y: cy + 48, r: 12, c: "#fbbf24", t: "Gut" },
  ];
  organs.forEach((o, i) => {
    ctx.globalAlpha = 0.55 + heat * 0.4;
    ctx.fillStyle = o.c;
    ctx.beginPath(); ctx.arc(o.x, o.y, o.r + (glow ? 2 : 0), 0, Math.PI * 2); ctx.fill();
    ctx.globalAlpha = 1;
    if (glow) {
      ctx.fillStyle = "#e2e8f0"; ctx.font = "600 10px Segoe UI"; ctx.textAlign = "center";
      ctx.fillText(o.t, o.x, o.y - o.r - 6);
    }
  });
  ctx.fillStyle = "#e2e8f0"; ctx.font = "700 13px Segoe UI"; ctx.textAlign = "center";
  ctx.fillText(glow ? "Organ team" : "Body map", cx, cy + 90);
}


export function registerBodyScenes(arena) {
  ...
  arena.registerScene(P + "Meet", (api) => {
    ...
    setTick(() => {
      ...
      const tips = {
        desk: "Meet heart, lungs, brain, and stomach on the map.",
        glow: "Each organ has a job - and they help each other.",
        settle: "Your body is a team of organs and support parts.",
      };
  ...
  arena.registerScene(P + "Sort", (api) => {
    ...
    const chips = [
      { id: "heart", short: "Heart", color: 0xfb7185 },
      { id: "lungs", short: "Lungs", color: 0x7dd3fc },
      { id: "brain", short: "Brain", color: 0xf9a8d4 },
      { id: "stomach", short: "Stomach", color: 0xfbbf24 },
      { id: "bone", short: "Bone", color: 0xe2e8f0 },
      { id: "muscle", short: "Muscle", color: 0xf87171 },
      { id: "phone", short: "Phone", color: 0x64748b },
      { id: "shoe", short: "Shoe", color: 0x78716c },
    ];
    const accept = {
      organ: ["heart", "lungs", "brain", "stomach"],
      support: ["bone", "muscle"],
      not: ["phone", "shoe"],
    };
    ...
    setTick(() => {
      ...
      const zones = [
        { id: "organ", label: "Organ", x: w * 0.02, y: zoneY, ww: w * 0.30, hh: zoneH, color: "#fb7185" },
        { id: "support", label: "Support/move", x: w * 0.34, y: zoneY, ww: w * 0.30, hh: zoneH, color: "#f87171" },
        { id: "not", label: "Not body part", x: w * 0.66, y: zoneY, ww: w * 0.32, hh: zoneH, color: "#94a3b8" },
      ];
      lastZones = zones;
      const hits = [];
      ...
      const byZone = { organ: [], support: [], not: [] };
      ...
      drawLabel(ctx, "Body Map sort", w * 0.5, layout.labelY);
  ...
  arena.registerScene(P + "Lab", (api) => {
    ...
    setTick(() => {
      ...
      ctx.fillStyle = "#fb7185"; ctx.beginPath(); ctx.arc(hx, h * 0.72, 14, 0, Math.PI * 2); ctx.fill();
      const msg = heat > 0.75 ? "Team locked" : heat > 0.55 ? "Map clear" : heat > 0.3 ? "Organs appearing" : "Map blurry";
  ...
  arena.registerScene(P + "Rule", (api) => {
    ...
    setTick(() => {
      ...
      drawLabel(ctx, "Body Map rule", w * 0.5, layout.labelY);
      failFlash(ctx, w, h); successFlash(ctx, w, h);
    });
    setDispose(() => {});
  });
  ...
  arena.registerScene(P + "Myth", (api) => {
    const { ctx, setTick, setDispose, setDescription, drawBackdrop, setHitRegions, setIntentHandler } = api;
    const myths = [
      { claim: "Only the heart matters", truth: "Organs team up - heart, lungs, brain, stomach, and more" },
      { claim: "Bones do nothing", truth: "Bones support and protect" },
      { claim: "The brain is not an organ", truth: "The brain is a key organ that controls the body" },
      { claim: "Phones are body organs", truth: "Gadgets are tools - not body organs" },
      { claim: "Organs work alone forever", truth: "Organs cooperate as a system" },
    ];
  ...
  arena.registerScene(P + "Drill", (api) => {
    ...
    setTick(() => {
      ...
      drawBackdrop();
      drawLabel(ctx, labState.prompt || "Body Map drill", w * 0.5, h * 0.2, { h: 32, font: "700 16px Segoe UI" });
  ...
  arena.registerScene(P + "Mastery", (api) => {
    ...
    setTick(() => {
      ...
      drawLabel(ctx, "Body Map mastery!", w * 0.5, layout.labelY);
      failFlash(ctx, w, h); successFlash(ctx, w, h);
    });
    setDispose(() => {});
  });
