/**
 * Heart Beat - Canvas 2D scenes (deepened).
 */
...
function drawHero(ctx, w, h, heat, phase) {
 ...
 const beat = 1 + Math.sin(performance.now() / 200) * 0.08 * (0.4 + heat);
 const glow = phase === "glow" || phase === "settle";
 ctx.save();
 ctx.translate(cx, cy);
 ctx.scale(beat, beat);
 ctx.fillStyle = glow ? "#fb7185" : "#e11d48";
 ctx.beginPath();
 ctx.moveTo(0, 16);
 ctx.bezierCurveTo(-40, -10, -30, -40, 0, -24);
 ctx.bezierCurveTo(30, -40, 40, -10, 0, 16);
 ctx.fill();
 ctx.restore();
 // vessels
 ctx.strokeStyle = `rgba(248,113,113,${0.4 + heat * 0.5})`;
 ctx.lineWidth = 4;
 ctx.beginPath(); ctx.moveTo(cx, cy + 30); ctx.quadraticCurveTo(cx + 60, cy + 40, cx + 90, cy + 10); ctx.stroke();
 ctx.beginPath(); ctx.moveTo(cx, cy + 30); ctx.quadraticCurveTo(cx - 60, cy + 40, cx - 90, cy + 10); ctx.stroke();
 ctx.fillStyle = "#e2e8f0"; ctx.font = "700 13px Segoe UI"; ctx.textAlign = "center";
 ctx.fillText(glow ? "Pump -> vessels -> body" : "Heart pump", cx, cy + 80);
}


export function registerHeartScenes(arena) {
 ...
 setDescription("Heart Beat - meet");
 setTick(() => {
 ...
 const tips = {
 desk: "See the heart squeeze and push blood.",
 glow: "Blood carries oxygen from the lungs.",
 settle: "Pulse is the beat you can feel.",
 };
 ...
 const chips = [
 { id: "pump", short: "Pump", color: 0xfb7185 },
 { id: "blood", short: "Blood", color: 0xf87171 },
 { id: "o2", short: "Oxygen", color: 0x7dd3fc },
 { id: "pulse", short: "Pulse", color: 0xf9a8d4 },
 { id: "vessel", short: "Vessel", color: 0xef4444 },
 { id: "rest", short: "Rest", color: 0x94a3b8 },
 { id: "stone", short: "Stone", color: 0x78716c },
 { id: "wire", short: "Charger", color: 0x64748b },
 ];
 const accept = {
 circ: ["pump", "blood", "vessel", "o2"],
 feel: ["pulse", "rest"],
 not: ["stone", "wire"],
 };
 ...
 const zones = [
 { id: "circ", label: "Circulation", x: w * 0.02, y: zoneY, ww: w * 0.30, hh: zoneH, color: "#fb7185" },
 { id: "feel", label: "What you notice", x: w * 0.34, y: zoneY, ww: w * 0.30, hh: zoneH, color: "#f9a8d4" },
 { id: "not", label: "Not circulation", x: w * 0.66, y: zoneY, ww: w * 0.32, hh: zoneH, color: "#94a3b8" },
 ];
 lastZones = zones;
 const hits = [];
 ...
 drawLabel(ctx, "Heart Beat sort", w * 0.5, layout.labelY);
 ...
 const msg = heat > 0.75 ? "Pump locked" : heat > 0.55 ? "Pulse clear" : heat > 0.3 ? "Pump working" : "Pulse blurry";
 ...
 drawLabel(ctx, "Heart Beat rule", w * 0.5, layout.labelY);
 failFlash(ctx, w, h); successFlash(ctx, w, h);
 });
 setDispose(() => {});
 });
 ...
 const { ctx, setTick, setDispose, setDescription, drawBackdrop, setHitRegions, setIntentHandler } = api;
 const myths = [
 { claim: "The heart only makes feelings", truth: "The heart is a pump that moves blood" },
 { claim: "Blood does not carry oxygen", truth: "Blood carries oxygen from lungs to the body" },
 { claim: "You cannot feel a pulse", truth: "Pulse is the beat you can feel in wrist or neck" },
 { claim: "Rest never changes heart rate", truth: "Rest often slows the beat; activity can raise it" },
 { claim: "Chargers pump blood", truth: "Only the heart pumps blood in your body" },
 ];
 ...
 drawBackdrop();
 drawLabel(ctx, labState.prompt || "Heart Beat drill", w * 0.5, h * 0.2, { h: 32, font: "700 16px Segoe UI" });
 ...
 drawLabel(ctx, "Heart Beat mastery!", w * 0.5, layout.labelY);
 failFlash(ctx, w, h); successFlash(ctx, w, h);
 });
 setDispose(() => {});
 });
