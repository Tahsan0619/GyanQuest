/**
 * Human Anatomy & Health boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerBodyScenes } from "./body-scenes.js";
import { registerHeartScenes } from "./heart-scenes.js";
...
try {
 ...
 await initI18n({ localeStorageKey: manifest.localeKey || "gq-human-anatomy-locale" });
 ...
 if (coachName) coachName.textContent = manifest.coachName || "Coach Body";
 ...
 const storageKey = manifest.storageKey || "gq-human-anatomy-save-v2";

 if (!canvas) throw new Error("Missing #c3d canvas");

 const arena = createArena2D(canvas, { defaultScene: "bodyMeet" });
 window.__arena = arena;
 registerBodyScenes(arena);
 registerHeartScenes(arena);
 ...
 const legacy = !saved ? loadSave("gq-human-anatomy-save-v1") : null;
 ...
 gameTitle: manifest.title || "Human Anatomy & Health",
 missions: MISSIONS,
 completed: state.completed,
 forceAllLocked: false,
 unlockByProgress: false,
 subtitle: "Missions 1-2 are live (Canvas 2D). Body Map / Heart Beat.",
 ...
 playScene("bodyMeet", { phase: "settle" });
 overlay.innerHTML = `
 <div class="chem-card">
 ...
 const hints = [
 [
 "Meet heart, lungs, brain, stomach on the map.",
 "Dial until organ map is clear.",
 "Sort organ / support / not-body.",
 "Push teamwork clarity higher.",
 "Order air -> pump -> brain -> fuel.",
 "Build: Organs team with jobs.",
 "Tap pulse / breath / meal contexts.",
 "Bust myths that only the heart matters.",
 "Hit body fluency checks.",
 "Claim Body Mapper.",
 ],
 [
 "See the heart pump and vessels.",
 "Dial until pulse idea is clear.",
 "Sort circulation / notice / not.",
 "Push circulation clarity higher.",
 "Order oxygen -> pump -> vessels -> pulse.",
 "Build: Heart pumps oxygen blood.",
 "Tap rest vs run pulse contexts.",
 "Bust myths that chargers pump blood.",
 "Hit pulse fluency checks.",
 "Claim Pulse Pro.",
 ],
 [
 "Coming soon hint.",
 ...
 if (!confirm("Reset all Human Anatomy & Health progress?")) return;
 clearSave(storageKey);
 clearSave("gq-human-anatomy-save-v1");
 state.level = 0;
 state.sub = 0;
 ...
 document.documentElement.classList.add("humananatomy-live", "chem-l1-live");
