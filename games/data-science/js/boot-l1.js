/**
 * Data Science boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerChartScenes } from "./chart-scenes.js";
...
export async function bootDataScience({ manifest }) {

const BOOKS = [BOOK_L1];
let bookApi = { onBookClick() { console.warn("[books] unavailable"); } };
try {
 bookApi = setupMissionBooks({
 subject: manifest.title || "data-science",
 ...
 await initI18n({ localeStorageKey: manifest.localeKey || "gq-data-science-locale" });
 ...
 if (coachName) coachName.textContent = manifest.coachName || "Coach Data";
 ...
 const storageKey = manifest.storageKey || "gq-data-science-save-v2";

 if (!canvas) throw new Error("Missing #c3d canvas");

 const arena = createArena2D(canvas, { defaultScene: "chartMeet" });
 window.__arena = arena;
 registerChartScenes(arena);
 ...
 const legacy = !saved ? loadSave("gq-data-science-save-v1") : null;
 ...
 gameTitle: manifest.title || "Data Science",
 missions: MISSIONS,
 completed: state.completed,
 forceAllLocked: false,
 unlockByProgress: false,
 subtitle: "Mission 1 is live (Canvas 2D). Chart Stories.",
 ...
 const introScene =
 state.level === 0
 ? ["chartMeet", { phase: "desk", dwellMs: 3200 }]
 : ["chartMeet", { phase: "settle", dwellMs: 2400 }];
 ...
 playScene("chartMeet", { phase: "settle" });
 overlay.innerHTML = `
 <div class="chem-card">
 ...
 if (!confirm("Reset all Data Science progress?")) return;
 clearSave(storageKey);
 clearSave("gq-data-science-save-v1");
 state.level = 0;
 state.sub = 0;
 ...
 document.documentElement.classList.add("datascience-live", "chem-l1-live");
