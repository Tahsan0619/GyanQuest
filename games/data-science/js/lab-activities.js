...
export function mountSpeedDrill(host, cfg) {
  ...
  function render() {
    if (idx >= items.length) {
      ...
      playScene(cfg.passScene || cfg.scene || "chartMastery");
      host.innerHTML = `
        <div class="chem-card">
    ...
    playScene(cfg.scene || "chartDrill", { prompt: chemLabState.prompt, flashColor: chemLabState.flashColor });
    host.innerHTML = `
...
export function mountMythCards(host, cfg) {
  ...
  function show() {
    ...
    playScene(cfg.scene || "chartMyth", { myth: m.sceneMyth });
    host.innerHTML = `
