/**
 * Electrical Basics activity mounts - panel + canvas share labState / intents.
 */
...
export function mountHeatLab(host, cfg) {
 ...
 // Defaults keep Circuit Loop framing; Mission labs pass topic overrides.
 const sliderLabel = cfg.sliderLabel || "Loop close";
 ...
 ${badgeHtml(cfg.badge || ATOM_ASSET_PATHS.m1, "circuit lab")}
 <h3>${cfg.title}</h3>
 <p>${cfg.html}</p>
...
export function mountScaleLab(host, cfg) {
 ...
 // Defaults keep Circuit Loop framing; Mission 1 passes topic overrides.
 const sliderLabel = cfg.sliderLabel || "Loop scale: torch \u2192 parts \u2192 CLOSED LOOP";
 const goalText = cfg.goalText || "Left canvas follows: everyday torch \u2192 kit parts \u2192 CLOSED LOOP rule.";
...
export function mountSpeedDrill(host, cfg) {
 ...
 playScene(cfg.passScene || cfg.scene || "circuitMastery");
 host.innerHTML = `
 <div class="chem-card">
 ...
 playScene(cfg.scene || "circuitDrill", { prompt: chemLabState.prompt, flashColor: chemLabState.flashColor });
 host.innerHTML = `
...
export function mountMythCards(host, cfg) {
 ...
 playScene(cfg.scene || "circuitMyth", { myth: m.sceneMyth });
 host.innerHTML = `
...
export function mountSpiralMap(host, cfg) {
 ...
 playScene(cfg.scene || "circSpiral");
 ...
 <button type="button" class="btn primary tiny-pulse" id="spiral-go">${cfg.finishLabel || "Finish Circuit Loop ▶"}</button>
 </div>`;
 function playStop(n) {
 if (cancelled) return;
 chemLabState.spiralStop = n;
 chemLabState.spiralUntil = performance.now() + 4500;
 playScene(cfg.scene || "circSpiral", { spiralStop: n });
 }
