/**
 * Electrical Basics boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerCircuitScenes } from "./circuit-scenes.js";
...
try {
  ...
  await initI18n({ localeStorageKey: manifest.localeKey || "gq-electrical-basics-locale" });
  ...
  if (coachName) coachName.textContent = manifest.coachName || "Coach Volt";
  ...
  const storageKey = manifest.storageKey || "gq-electrical-basics-save-v2";
  ...
  const legacy = !saved ? loadSave("gq-electrical-basics-save-v1") : null;
  ...
  function showHub() {
    ...
    mountMissionHub(hubRoot, {
      ...
      gameTitle: manifest.title || "Electrical Basics",
      missions: MISSIONS,
      completed: state.completed,
      forceAllLocked: false,
      unlockByProgress: false,
      subtitle: "Missions 1–3 are live (Canvas 2D). Circuit Loop  -  Voltage & Current  -  Safe Power.",
  ...
  btnResetAll?.addEventListener("click", () => {
    if (!confirm("Reset all Electrical Basics progress?")) return;
    clearSave(storageKey);
    clearSave("gq-electrical-basics-save-v1");
    state.level = 0;
    state.sub = 0;
  ...
  document.documentElement.classList.add("electricalba-live", "chem-l1-live");
