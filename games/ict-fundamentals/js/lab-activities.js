...
export function mountHeatLab(host, cfg) {
 ...
 // Defaults keep ICT framing; Computer Bits / I/O pass topic overrides.
 const sliderLabel = cfg.sliderLabel || "Work / signal level";
 ...
 ${badgeHtml(cfg.badge || ATOM_ASSET_PATHS.life, "ict lab")}
 <h3>${cfg.title}</h3>
 <p>${cfg.html}</p>
 ...
 cold: "Idle - waiting for work or signal",
 melting: "Rising - more activity",
 liquid: "Busy - goal nearly met",
 simmer: "Full - goal reached",
 };
...
export function mountScaleLab(host, cfg) {
 ...
 // Defaults keep ICT framing; Computer Bits / I/O pass topic overrides.
 const sliderLabel = cfg.sliderLabel || "ICT scale: desk -> detail -> rule";
 const goalText = cfg.goalText || "Left canvas follows: desk parts -> closer look -> rule banner.";
 const readouts = cfg.readoutLabels || {
 low: "Desk view - parts together",
 mid: "Closer detail - signal or open work",
...
export function mountSpeedDrill(host, cfg) {
 ...
 playScene(cfg.passScene || cfg.scene || "bitsMastery");
 host.innerHTML = `
 <div class="chem-card">
 ...
 playScene(cfg.scene || "bitsDrill", { prompt: chemLabState.prompt, flashColor: chemLabState.flashColor });
 host.innerHTML = `
...
export function mountMythCards(host, cfg) {
 ...
 playScene(cfg.scene || "bitsMyth", { myth: m.sceneMyth });
 host.innerHTML = `
...
export function signalGateReady(opts = {}) {
 if (!activeGate || activeGate.cancelled) return;
 const { btn, status, cfg, finish } = activeGate;
...
export function mountSpiralMap(host, cfg) {
 ...
 playScene(cfg.scene || "bitsSpiral");
 ...
 <button type="button" class="btn primary tiny-pulse" id="spiral-go">${cfg.finishLabel || "Finish Computer Bits ▶"}</button>
 </div>`;
 function playStop(n) {
 if (cancelled) return;
 chemLabState.spiralStop = n;
 chemLabState.spiralUntil = performance.now() + 4500;
 playScene(cfg.scene || "bitsSpiral", { spiralStop: n });
 }
