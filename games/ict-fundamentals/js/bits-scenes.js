/**
 * ICT / Mission 1: Computer Bits - CPU, RAM, storage (topic-fit visuals).
 */
...
function drawRam(ctx, x, y, fill = 1) {
 ctx.fillStyle = "#14532d";
 roundRect(ctx, x - 50, y - 14, 100, 28, 4);
 ctx.fill();
 ctx.fillStyle = "#22c55e";
 roundRect(ctx, x - 50, y - 14, 100 * Math.max(0.08, Math.min(1, fill)), 28, 4);
 ctx.fill();
 for (let i = 0; i < 8; i++) {
...
function drawPhone(ctx, x, y) {
 ctx.fillStyle = "#0f172a";
 roundRect(ctx, x - 28, y - 48, 56, 96, 12);
 ctx.fill();
...
function drawLaptop(ctx, x, y) {
 ctx.fillStyle = "#1e293b";
 roundRect(ctx, x - 70, y - 50, 140, 70, 6);
 ctx.fill();
 ctx.fillStyle = "#38bdf8";
 roundRect(ctx, x - 62, y - 42, 124, 54, 4);
 ctx.fill();
 ctx.fillStyle = "#334155";
 roundRect(ctx, x - 80, y + 22, 160, 12, 4);
 ctx.fill();
}
function drawTower(ctx, x, y) {
 ctx.fillStyle = "#1e293b";
 roundRect(ctx, x - 36, y - 55, 72, 110, 8);
 ctx.fill();
 ctx.fillStyle = "#60a5fa";
 roundRect(ctx, x - 24, y - 40, 48, 18, 4);
 ctx.fill();
 for (let i = 0; i < 4; i++) {
 ctx.fillStyle = "#475569";
 ctx.fillRect(x - 24, y - 10 + i * 14, 48, 6);
 }
}
function drawConsole(ctx, x, y) {
 ctx.fillStyle = "#312e81";
 roundRect(ctx, x - 55, y - 20, 110, 40, 12);
 ctx.fill();
...
function drawTablet(ctx, x, y) {
 ctx.fillStyle = "#0f172a";
 roundRect(ctx, x - 50, y - 36, 100, 72, 10);
 ctx.fill();
 ctx.fillStyle = "#22d3ee";
 roundRect(ctx, x - 42, y - 28, 84, 56, 6);
 ctx.fill();
}
...
export function registerBitsScenes(arena) {
 ...
 drawRam(ctx, props.ram.x, props.ram.y, live === "settle" ? 0.55 : 0.85);
 ...
 drawRam(ctx, w * 0.65, h * 0.4, heat);
 // Open-app tabs growing with workload
 const tabs = Math.max(1, Math.round(heat * 5));
 for (let i = 0; i < tabs; i++) {
 ctx.fillStyle = `rgba(34,197,94,${0.35 + i * 0.1})`;
 roundRect(ctx, w * 0.52 + i * 18, h * 0.22, 40, 16, 4);
 ctx.fill();
 ctx.fillStyle = "#ecfdf5";
 ctx.font = "600 9px Segoe UI";
 ctx.textAlign = "center";
 ctx.fillText("app", w * 0.52 + i * 18 + 20, h * 0.22 + 11);
 }
 const hx = w * 0.2 + heat * w * 0.6;
 ctx.fillStyle = "#60a5fa";
 ctx.beginPath(); ctx.arc(hx, h * 0.7, 14, 0, Math.PI * 2); ctx.fill();
 ctx.fillStyle = "rgba(148,163,184,0.5)";
 ctx.fillRect(w * 0.2, h * 0.7 - 3, w * 0.6, 6);
 ...
 drawBackdrop();
 // Scale scrubber owns the canvas once the dial moves; otherwise show equation tokens.
 if (scale > 0.02) {
 if (scale < 0.33) {
 drawCpu(ctx, w * 0.28, h * 0.42, 0.4);
 drawRam(ctx, w * 0.5, h * 0.48, 0.5);
 drawDisk(ctx, w * 0.72, h * 0.42);
 drawLabel(ctx, "Desk PC - whole inside team", w * 0.5, layout.labelY);
 } else if (scale < 0.66) {
 drawCpu(ctx, w * 0.32, h * 0.4, 0.85);
 drawRam(ctx, w * 0.62, h * 0.42, 0.9);
 ctx.strokeStyle = "rgba(96,165,250,0.8)";
 ctx.lineWidth = 3;
 ctx.beginPath();
 ctx.moveTo(w * 0.32 + 40, h * 0.4);
 ctx.lineTo(w * 0.62 - 50, h * 0.42);
 ctx.stroke();
 for (let i = 0; i < 4; i++) {
 ctx.fillStyle = "rgba(34,197,94,0.55)";
 roundRect(ctx, w * 0.48 + i * 22, h * 0.22, 36, 14, 4);
 ctx.fill();
 }
 drawLabel(ctx, "Open work glowing in RAM", w * 0.5, layout.labelY);
 } else {
 drawDisk(ctx, w * 0.5, h * 0.38);
 ...
 ctx.textAlign = "center";
 ctx.fillText("STORAGE KEEPS after power off", w * 0.5, h * 0.6 + 14);
 drawLabel(ctx, "Computer Bits rule locked", w * 0.5, layout.labelY);
 }
 } else {
 ["CPU thinks", "RAM holds", "Storage keeps", "Power off"].forEach((label, i) => {
 const x = w * 0.16 + i * (w * 0.2);
 ctx.fillStyle = i < prog ? "rgba(96,165,250,0.4)" : "rgba(15,23,42,0.9)";
 roundRect(ctx, x - 48, h * 0.34 - 18, 96, 36, 10); ctx.fill();
 ctx.fillStyle = "#dbeafe"; ctx.font = "700 11px Segoe UI"; ctx.textAlign = "center";
 ctx.fillText(label, x, h * 0.34);
 });
 drawCpu(ctx, w * 0.35, h * 0.58, 0.6);
 drawRam(ctx, w * 0.55, h * 0.6, 0.7);
 drawDisk(ctx, w * 0.75, h * 0.58);
 drawLabel(ctx, "Build the Computer Bits rule", w * 0.5, layout.labelY);
 }
 failFlash(ctx, w, h); successFlash(ctx, w, h);
 });
 setDispose(() => {});
 ...
 if (mode === "phone") drawPhone(ctx, cx - 80, cy);
 else if (mode === "laptop") drawLaptop(ctx, cx - 70, cy);
 else if (mode === "lab") drawTower(ctx, cx - 80, cy);
 else if (mode === "game") drawConsole(ctx, cx - 70, cy);
 else drawTablet(ctx, cx - 70, cy);
 drawCpu(ctx, cx + 70, cy - 10, 0.55);
 drawRam(ctx, cx + 70, cy + 55, 0.7);
 drawDisk(ctx, cx + 150, cy + 10);
 const captions = {
 phone: "Phone: SoC + RAM + flash storage",
 ...
 // Per-myth diagram
 if (idx === 0) {
 drawRam(ctx, w * 0.32, h * 0.32, phase === "truth" ? 0.4 : 1);
 drawDisk(ctx, w * 0.68, h * 0.32);
 drawLabel(ctx, phase === "truth" ? "Different jobs" : "Same?", w * 0.5, h * 0.48, { h: 22, font: "700 12px Segoe UI" });
 } else if (idx === 1) {
 drawCpu(ctx, w * 0.5, h * 0.32, 0.8);
 ctx.fillStyle = "#dbeafe";
 ctx.font = "600 12px Segoe UI";
 ctx.textAlign = "center";
 ctx.fillText(phase === "truth" ? "School apps + games" : "Games only?", w * 0.5, h * 0.52);
 } else if (idx === 2) {
 drawDisk(ctx, w * 0.28, h * 0.32);
 if (phase === "truth") {
 drawCpu(ctx, w * 0.55, h * 0.3, 0.7);
 drawRam(ctx, w * 0.75, h * 0.36, 0.8);
 } else {
 ctx.fillStyle = "#fbbf24";
 ctx.font = "700 28px Segoe UI";
 ctx.textAlign = "center";
 ctx.fillText("BIG?", w * 0.65, h * 0.34);
 }
 } else if (idx === 3) {
 drawPhone(ctx, w * 0.38, h * 0.34);
 if (phase === "truth") drawCpu(ctx, w * 0.62, h * 0.34, 0.7);
 else {
 ctx.fillStyle = "#f87171";
 ctx.font = "700 22px Segoe UI";
 ctx.textAlign = "center";
 ctx.fillText("No CPU?", w * 0.65, h * 0.36);
 }
 } else {
 drawLaptop(ctx, w * 0.35, h * 0.3);
 drawDisk(ctx, w * 0.7, h * 0.34);
 ctx.fillStyle = phase === "truth" ? "#86efac" : "#fca5a5";
 ctx.font = "700 12px Segoe UI";
 ctx.textAlign = "center";
 ctx.fillText(phase === "truth" ? "Files stay on storage" : "Lid closes = delete?", w * 0.5, h * 0.52);
 }
 ctx.fillStyle = phase === "truth" ? "rgba(96,165,250,0.2)" : "rgba(248,113,113,0.18)";
 roundRect(ctx, w * 0.1, h * 0.58, w * 0.8, 52, 12);
 ctx.fill();
 drawLabel(ctx, phase === "truth" ? m.truth : "Myth: " + m.claim, w * 0.5, h * 0.6 + 16, { h: 36, font: "700 12px Segoe UI", maxW: w * 0.75 });
 drawLabel(ctx, "Myth " + (idx + 1) + " / 5", w * 0.5, layout.labelY);
 setHitRegions([{ id: "card", shape: "rect", x: w * 0.5, y: h * 0.45, w: w * 0.8, h: h * 0.4, meta: { action: "flip" } }]);
 failFlash(ctx, w, h); successFlash(ctx, w, h);
 });
 setDispose(() => setIntentHandler(null));
 });
 ...
 drawBackdrop();
 drawLabel(ctx, labState.prompt || "Computer Bits drill", w * 0.5, h * 0.16, { h: 32, font: "700 16px Segoe UI" });
 if (p.includes("ram") || p.includes("open")) drawRam(ctx, w * 0.5, h * 0.45, 0.75);
 else if (p.includes("save") || p.includes("ssd") || p.includes("storage")) drawDisk(ctx, w * 0.5, h * 0.45);
 else if (p.includes("phone")) { drawPhone(ctx, w * 0.38, h * 0.42); drawCpu(ctx, w * 0.62, h * 0.42, 0.6); }
 else if (p.includes("snack")) {
 ctx.fillStyle = "#f97316";
 roundRect(ctx, w * 0.5 - 40, h * 0.4, 80, 40, 10);
 ctx.fill();
 ctx.fillStyle = "#fff7ed";
 ctx.font = "700 14px Segoe UI";
 ctx.textAlign = "center";
 ctx.fillText("Not a PC part", w * 0.5, h * 0.42 + 14);
 } else if (p.includes("rule")) {
 drawCpu(ctx, w * 0.3, h * 0.45, 0.5);
 drawRam(ctx, w * 0.5, h * 0.48, 0.6);
 drawDisk(ctx, w * 0.72, h * 0.45);
 } else drawCpu(ctx, w * 0.5, h * 0.45, 0.6);
 failFlash(ctx, w, h); successFlash(ctx, w, h);
 });
 setDispose(() => {});
 });
 ...
 drawPhone(ctx, w * 0.18, h * 0.36);
 drawCpu(ctx, w * 0.4, h * 0.34, 0.8);
 drawRam(ctx, w * 0.58, h * 0.4, 0.7);
 drawDisk(ctx, w * 0.78, h * 0.36);
 ...
 ctx.textAlign = "center";
 ctx.fillText("Bit Scout", w * 0.5, h * 0.58 + 24);
 drawLabel(ctx, "Phone + PC: same inside team", w * 0.5, layout.labelY);
 failFlash(ctx, w, h); successFlash(ctx, w, h);
 });
 setDispose(() => {});
 });
