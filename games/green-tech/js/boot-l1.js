/**
 * Green Tech boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerCleanScenes } from "./clean-scenes.js";
...
try {
  ...
  await initI18n({ localeStorageKey: manifest.localeKey || "gq-green-tech-locale" });
  ...
  if (coachName) coachName.textContent = manifest.coachName || "Coach Green";
  ...
  const storageKey = manifest.storageKey || "gq-green-tech-save-v2";

  if (!canvas) throw new Error("Missing #c3d canvas");

  const arena = createArena2D(canvas, { defaultScene: "cleanMeet" });
  window.__arena = arena;
    registerCleanScenes(arena);
  ...
  const legacy = !saved ? loadSave("gq-green-tech-save-v1") : null;
  ...
  function showHub() {
    ...
    mountMissionHub(hubRoot, {
      ...
      gameTitle: manifest.title || "Green Tech",
      missions: MISSIONS,
      completed: state.completed,
      forceAllLocked: false,
      unlockByProgress: false,
      subtitle: "Mission 1 is live (Canvas 2D). Clean Energy - sun and wind power.",
  ...
  function openIntro(then) {
    ...
    const introScene =
      state.level === 0
        ? ["cleanMeet", { phase: "desk", dwellMs: 3200 }]
        : state.level === 1
          ? ["cleanMeet", { phase: "desk", dwellMs: 3200 }]
          : state.level === 2
            ? ["cleanMeet", { phase: "desk", dwellMs: 3200 }]
            : ["cleanMeet", { phase: "settle", dwellMs: 2400 }];
  ...
  function runComingSoon() {
    ...
    playScene("cleanMeet", { phase: "settle" });
    overlay.innerHTML = `
      <div class="chem-card">
  ...
  btnHint?.addEventListener("click", () => {
    const hints = [
      [
        "Meet sun, turbine, and smoky stack.",
        "Dial clean share until smoke fades.",
        "Sort clean, smoky, and store/not.",
        "Push clean share higher.",
        "Order why clean helps.",
        "Build: Sun / wind power clean.",
        "Tap rooftop, yard, lamps, shop, lab.",
        "Bust clean-energy myths.",
  ...
  btnResetAll?.addEventListener("click", () => {
    if (!confirm("Reset all Green Tech progress?")) return;
    clearSave(storageKey);
    clearSave("gq-green-tech-save-v1");
    state.level = 0;
    state.sub = 0;
  ...
  document.documentElement.classList.add("greentech-live", "chem-l1-live");
