...
export function registerPushScenes(arena) {
  ...
  arena.registerScene("pushStretch", (api) => {
    ...
    setTick(() => {
      ...
      drawCar(ctx, w * 0.5, h * 0.36, mode === "bike" ? 0.7 : 1.1, mode === "sofa" ? "#a78bfa" : "#f97316");
      const captions = {
  ...
  arena.registerScene("pushMyth", (api) => {
    const { ctx, setTick, setDispose, setDescription, drawBackdrop, setHitRegions, setIntentHandler } = api;
    const myths = [
      { claim: "Heavier things always need more force to move at all", truth: "Any net F accelerates; heavier just gets smaller a for the same F" },
      { claim: "Force is the same as speed", truth: "Force changes velocity (acceleration); speed is not force" },
      { claim: "F = m + a", truth: "F = m · a (multiply, don’t add)" },
      { claim: "Zero force means zero velocity", truth: "Zero net force means constant velocity (including rest)" },
      { claim: "Only rockets use F=ma", truth: "Every net force and mass pair follows it" },
    ];
  ...
  arena.registerScene("pushDrill", (api) => {
    ...
    setTick(() => {
      ...
      const h = api.height;
      drawBackdrop();
      drawLabel(ctx, forceLabState.prompt || "F = m · a drill", w * 0.5, h * 0.2, { h: 32, font: "700 16px Segoe UI" });
      drawCar(ctx, w * 0.5, h * 0.5, 1.1);
      failFlash(ctx, w, h);
      successFlash(ctx, w, h);
    });
    setDispose(() => {});
