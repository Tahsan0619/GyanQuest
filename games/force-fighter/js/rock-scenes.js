...
export function registerRockScenes(arena) {
  ...
  arena.registerScene("rockMyth", (api) => {
    ...
    setDescription("Bust force myths - tap card to flip.");
    const myths = [
      { claim: "A kick keeps pushing the ball forever", truth: "The kick is a short force; after that inertia + friction rule" },
      { claim: "Moving things need a constant push to keep going", truth: "Only net force changes velocity - coasting needs none" },
      { claim: "Heavier objects fall because they want to stop more", truth: "Inertia resists change - not a desire; gravity is separate" },
      { claim: "If something is moving there must be a force on it now", truth: "It can coast with zero net force after the push ends" },
      { claim: "Rest means no forces at all", truth: "Rest can mean forces are balanced (net zero)" },
    ];
    setIntentHandler((intent) => {
    ...
    setTick(() => {
      ...
      drawRock(ctx, w * 0.5, h * 0.7, 0.8, phase === "truth");
  ...
  arena.registerScene("rockDrill", (api) => {
    ...
    setTick(() => {
      ...
      drawLabel(ctx, forceLabState.prompt || "Speed drill!", w * 0.5, h * 0.12, { h: 32, font: "700 16px Segoe UI" });
      drawRock(ctx, w * 0.5, h * 0.5, 1.1, true);
      failFlash(ctx, w, h);
      successFlash(ctx, w, h);
    });
    setDispose(() => {});
  ...
  arena.registerScene("rockMastery", (api) => {
    ...
    setTick(() => {
      ...
      drawRock(ctx, w * 0.5, h * 0.4, 1.2, true);
