...
export function registerPairScenes(arena) {
 ...
 const modeLabels = { balloon: "Balloon", rower: "Rower", weight: "Weight", bug: "Bug", tug: "Tug" };
 if (opts?.mode) forceLabState.mode = opts.mode;
 ...
 roundRect(ctx, w * 0.32, cy + 10, w * 0.36, 22, 8);
 ctx.fill();
 drawSkater(ctx, w * 0.45, cy, 1);
 drawArrow(ctx, w * 0.58, cy + 5, w * 0.75, cy + 5, "#f472b6");
 drawArrow(ctx, w * 0.4, cy - 8, w * 0.22, cy - 8, "#38bdf8");
 drawLabel(ctx, "oar→water", w * 0.7, cy + 30, { h: 16, font: "600 10px Segoe UI" });
 drawLabel(ctx, "water→boat", w * 0.28, cy - 30, { h: 16, font: "600 10px Segoe UI" });
 } else if (mode === "weight") {
 ctx.fillStyle = "#57534e";
 roundRect(ctx, w * 0.35, cy + 40, w * 0.3, 18, 4);
 ctx.fill();
 drawSkater(ctx, w * 0.5, cy, 1);
 drawArrow(ctx, w * 0.5, cy + 20, w * 0.5, cy + 55, "#f472b6");
 drawArrow(ctx, w * 0.5, cy + 55, w * 0.5, cy + 20, "#38bdf8");
 drawLabel(ctx, "You↔Earth", w * 0.5, cy - 50, { h: 18, font: "600 11px Segoe UI" });
 } else if (mode === "bug") {
 // glass + bug
 ctx.strokeStyle = "#94a3b8";
 ctx.lineWidth = 4;
 ctx.strokeRect(w * 0.35, cy - 50, w * 0.3, 100);
 ctx.fillStyle = "#22c55e";
 ctx.beginPath();
 ctx.arc(w * 0.5, cy, 10, 0, Math.PI * 2);
 ctx.fill();
 drawArrow(ctx, w * 0.5 - 30, cy, w * 0.5 - 12, cy, "#f472b6");
 drawArrow(ctx, w * 0.5 + 30, cy, w * 0.5 + 12, cy, "#38bdf8");
 drawLabel(ctx, "Equal magnitudes", w * 0.5, cy + 70, { h: 18, font: "600 11px Segoe UI" });
 } else {
 // tug
 drawSkater(ctx, w * 0.32, cy, 1);
 drawSkater(ctx, w * 0.68, cy, -1);
 ctx.strokeStyle = "#d6d3d1";
 ctx.lineWidth = 3;
 ctx.beginPath();
 ctx.moveTo(w * 0.36, cy);
 ctx.lineTo(w * 0.64, cy);
 ctx.stroke();
 drawArrow(ctx, w * 0.48, cy - 30, w * 0.38, cy - 30, "#f472b6");
 drawArrow(ctx, w * 0.52, cy - 30, w * 0.62, cy - 30, "#38bdf8");
 }
 const captions = {
 ...
 const { ctx, setTick, setDispose, setDescription, drawBackdrop, setHitRegions, setIntentHandler } = api;
 const myths = [
 { claim: "The bigger object wins the force pair", truth: "Pair forces equal in size; a differs by mass", kind: "bigger" },
 { claim: "Action happens first, then reaction", truth: "Simultaneous - one interaction", kind: "later" },
 { claim: "A mid-rope scale should read zero", truth: "Reads tension (~one side), not zero", kind: "scale" },
 { claim: "You don’t push the Earth when you jump", truth: "You push Earth down; Earth pushes you up", kind: "jump" },
 { claim: "Pairs cancel so nothing can move", truth: "Each force acts on a different object", kind: "cancel" },
 ];
 ...
 } else if (m.kind === "bigger") {
 drawSkater(ctx, cx - 70, cy, 1);
 ctx.fillStyle = "#64748b";
 roundRect(ctx, cx + 40, cy - 30, 50, 60, 6);
 ctx.fill();
 drawArrow(ctx, cx - 40, cy - 20, cx - 10, cy - 20, "#f472b6");
 drawArrow(ctx, cx + 30, cy - 20, cx, cy - 20, "#38bdf8");
 drawLabel(ctx, "Equal F · different a", cx, cy + 60, { h: 18, font: "600 11px Segoe UI" });
 } else if (m.kind === "later") {
 drawLabel(ctx, "Same instant", cx, cy - 10, { h: 28, font: "700 16px Segoe UI" });
 drawArrow(ctx, cx - 50, cy + 30, cx - 10, cy + 30, "#f472b6");
 drawArrow(ctx, cx + 50, cy + 30, cx + 10, cy + 30, "#38bdf8");
 ...
 ctx.stroke();
 ctx.fillStyle = "#fef3c7";
 roundRect(ctx, cx - 36, cy - 22, 72, 36, 8);
 ctx.fill();
 ctx.fillStyle = "#1c1917";
 ctx.font = "700 14px Segoe UI";
 ctx.textAlign = "center";
 ctx.fillText("~120 N", cx, cy - 2);
 } else if (m.kind === "jump") {
 drawSkater(ctx, cx, cy - 20, 1);
 ctx.fillStyle = "#57534e";
 ctx.fillRect(w * 0.25, cy + 40, w * 0.5, 14);
 drawArrow(ctx, cx, cy + 20, cx, cy + 45, "#f472b6");
 drawArrow(ctx, cx, cy + 45, cx, cy + 20, "#38bdf8");
 } else {
 drawSkater(ctx, cx - 50, cy, 1);
 drawSkater(ctx, cx + 50, cy, -1);
 drawLabel(ctx, "Two bodies · don’t cancel on one", cx, cy + 60, { h: 18, font: "600 11px Segoe UI" });
 }
 ...
 drawBackdrop();
 drawLabel(ctx, forceLabState.prompt || "Equal & opposite!", w * 0.5, h * 0.14, { h: 28, font: "700 15px Segoe UI" });
 const cx = w * 0.5;
 const cy = h * 0.48;
 if (p.includes("balloon")) {
 ctx.fillStyle = "#f472b6";
 ctx.beginPath();
 ctx.ellipse(cx, cy, 28, 36, 0, 0, Math.PI * 2);
 ctx.fill();
 drawArrow(ctx, cx, cy + 36, cx, cy + 70, "#f472b6");
 drawArrow(ctx, cx, cy - 36, cx, cy - 70, "#38bdf8");
 ...
 // Showcase trio: skaters, rocket, rope scale
 drawSkater(ctx, w * 0.22, h * 0.36, 1);
 drawSkater(ctx, w * 0.34, h * 0.36, -1);
 ctx.fillStyle = "#e2e8f0";
 roundRect(ctx, w * 0.5 - 10, h * 0.3, 20, 45, 4);
 ctx.fill();
 ctx.strokeStyle = "#d6d3d1";
 ctx.lineWidth = 3;
 ctx.beginPath();
 ...
 ctx.stroke();
 ctx.fillStyle = "#fef3c7";
 roundRect(ctx, w * 0.74, h * 0.36, 40, 24, 6);
 ctx.fill();
