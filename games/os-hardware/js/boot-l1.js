/**
 * OS & Hardware boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerBoxScenes } from "./box-scenes.js";
...
export async function bootOsHardware({ manifest }) {

const BOOKS = [BOOK_L1];
let bookApi = { onBookClick() { console.warn("[books] unavailable"); } };
try {
 bookApi = setupMissionBooks({
 subject: manifest.title || "os-hardware",
 ...
 await initI18n({ localeStorageKey: manifest.localeKey || "gq-os-hardware-locale" });
 ...
 if (coachName) coachName.textContent = manifest.coachName || "Coach Kernel";
 ...
 const storageKey = manifest.storageKey || "gq-os-hardware-save-v2";

 if (!canvas) throw new Error("Missing #c3d canvas");

 const arena = createArena2D(canvas, { defaultScene: "boxMeet" });
 window.__arena = arena;
 registerBoxScenes(arena);
 ...
 const legacy = !saved ? loadSave("gq-os-hardware-save-v1") : null;
 ...
 gameTitle: manifest.title || "OS & Hardware",
 missions: MISSIONS,
 completed: state.completed,
 forceAllLocked: false,
 unlockByProgress: false,
 subtitle: "Mission 1 is live (Canvas 2D). Inside the Box.",
 ...
 const introScene =
 state.level === 0
 ? ["boxMeet", { phase: "desk", dwellMs: 3200 }]
 : ["boxMeet", { phase: "settle", dwellMs: 2400 }];
 ...
 playScene("boxMeet", { phase: "settle" });
 overlay.innerHTML = `
 <div class="chem-card">
 ...
 if (!confirm("Reset all OS & Hardware progress?")) return;
 clearSave(storageKey);
 clearSave("gq-os-hardware-save-v1");
 state.level = 0;
 state.sub = 0;
 ...
 document.documentElement.classList.add("oshardware-live", "chem-l1-live");
