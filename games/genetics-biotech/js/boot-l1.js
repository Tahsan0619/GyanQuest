/**
 * Genetics & Biotech boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerTraitScenes } from "./trait-scenes.js";
...
export async function bootGeneticsBiotech({ manifest }) {

const BOOKS = [BOOK_L1];
let bookApi = { onBookClick() { console.warn("[books] unavailable"); } };
try {
  bookApi = setupMissionBooks({
  subject: manifest.title || "genetics-biotech",
  ...
  await initI18n({ localeStorageKey: manifest.localeKey || "gq-genetics-biotech-locale" });
  ...
  if (coachName) coachName.textContent = manifest.coachName || "Coach Gene";
  ...
  const storageKey = manifest.storageKey || "gq-genetics-biotech-save-v2";

  if (!canvas) throw new Error("Missing #c3d canvas");

  const arena = createArena2D(canvas, { defaultScene: "traitMeet" });
  window.__arena = arena;
    registerTraitScenes(arena);
  ...
  const legacy = !saved ? loadSave("gq-genetics-biotech-save-v1") : null;
  ...
  function showHub() {
    ...
    mountMissionHub(hubRoot, {
      ...
      gameTitle: manifest.title || "Genetics & Biotech",
      missions: MISSIONS,
      completed: state.completed,
      forceAllLocked: false,
      unlockByProgress: false,
      subtitle: "Mission 1 is live (Canvas 2D). Trait Tokens.",
  ...
  function openIntro(then) {
    ...
    const introScene =
      state.level === 0
        ? ["traitMeet", { phase: "desk", dwellMs: 3200 }]
        : state.level === 1
          ? ["pushMeet", { phase: "predict", dwellMs: 3200 }]
          : state.level === 2
            ? ["pairMeet", { phase: "desk", dwellMs: 3200 }]
            : ["traitMeet", { phase: "settle", dwellMs: 2400 }];
  ...
  function runComingSoon() {
    ...
    playScene("traitMeet", { phase: "settle" });
    overlay.innerHTML = `
      <div class="chem-card">
  ...
  btnHint?.addEventListener("click", () => {
    const hints = [
      [
        "Meet eye, hair, dimple trait tokens.",
        "Dial until inherit idea is clear.",
        "Sort can-inherit vs not-inherited.",
        "Push family-clue clarity higher.",
        "Order parents -> pass -> show -> not-scar.",
        "Build: Genes can pass traits.",
        "Tap family eye-color contexts.",
        "Bust myths that scars are gene traits.",
        "Hit trait fluency checks.",
        "Claim Trait Tracker.",
      ],
      [
        "Coming soon hint.",
  ...
  btnResetAll?.addEventListener("click", () => {
    if (!confirm("Reset all Genetics & Biotech progress?")) return;
    clearSave(storageKey);
    clearSave("gq-genetics-biotech-save-v1");
    state.level = 0;
    state.sub = 0;
  ...
  document.documentElement.classList.add("geneticsbiot-live", "chem-l1-live");
