/**
 * Trait Tokens - Canvas 2D scenes (deepened).
 */
...
function drawLabel(ctx, text, x, y, opts = {}) {
  ...
  ctx.fill();
  ctx.strokeStyle = opts.border || "#67e8f9";
  ctx.lineWidth = 1.4;
  ctx.stroke();
  ctx.fillStyle = opts.color || "#cffafe";
...
function drawHero(ctx, w, h, heat, phase) {
  ...
  const tokens = [
    { x: w * 0.25, t: "Eyes", c: "#67e8f9" },
    { x: w * 0.5, t: "Hair", c: "#22d3ee" },
    { x: w * 0.75, t: "Dimple", c: "#a5f3fc" },
  ];
  // parent -> child arrows
  ctx.strokeStyle = `rgba(103,232,249,${0.35 + heat * 0.5})`;
  ctx.lineWidth = 3;
  ctx.beginPath(); ctx.moveTo(w * 0.2, h * 0.28); ctx.lineTo(w * 0.35, h * 0.48); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(w * 0.8, h * 0.28); ctx.lineTo(w * 0.65, h * 0.48); ctx.stroke();
  ctx.fillStyle = "#fbbf24";
  roundRect(ctx, w * 0.12, h * 0.2, 70, 36, 8); ctx.fill();
  roundRect(ctx, w * 0.7, h * 0.2, 70, 36, 8); ctx.fill();
  ctx.fillStyle = "#0f172a"; ctx.font = "700 11px Segoe UI"; ctx.textAlign = "center";
  ctx.fillText("Parent", w * 0.22, h * 0.24);
  ctx.fillText("Parent", w * 0.8, h * 0.24);
  tokens.forEach((tok) => {
    ctx.fillStyle = tok.c;
    roundRect(ctx, tok.x - 34, h * 0.48, 68, 40, 10); ctx.fill();
    if (glow) { ctx.strokeStyle = "#ecfeff"; ctx.lineWidth = 2; ctx.stroke(); }
    ctx.fillStyle = "#0f172a"; ctx.fillText(tok.t, tok.x, h * 0.52);
  });
  ctx.fillStyle = "#e2e8f0"; ctx.font = "700 13px Segoe UI";
  ctx.fillText(glow ? "Traits can pass" : "Trait tokens", w * 0.5, h * 0.66);
}


export function registerTraitScenes(arena) {
  ...
  arena.registerScene(P + "Meet", (api) => {
    ...
    setDescription("Trait Tokens - meet");
    setTick(() => {
      ...
      const tips = {
        desk: "Meet eye, hair, and dimple clue tokens.",
        glow: "Some traits can come from parents via genes.",
        settle: "Learned skills and scars are not inherited traits.",
      };
  ...
  arena.registerScene(P + "Sort", (api) => {
    ...
    const chips = [
      { id: "eye", short: "Eyes", color: 0x67e8f9 },
      { id: "hair", short: "Hair", color: 0x22d3ee },
      { id: "dimple", short: "Dimple", color: 0xa5f3fc },
      { id: "parent", short: "Parents", color: 0xfbbf24 },
      { id: "gene", short: "Genes", color: 0x2dd4bf },
      { id: "learn", short: "Learned", color: 0x94a3b8 },
      { id: "scar", short: "Scar", color: 0x78716c },
      { id: "hat", short: "Hat", color: 0x64748b },
    ];
    const accept = {
      inherit: ["eye", "hair", "dimple", "parent", "gene"],
      not: ["learn", "scar", "hat"],
    };
    ...
    setTick(() => {
      ...
      const zones = [
        { id: "inherit", label: "Can inherit", x: w * 0.02, y: zoneY, ww: w * 0.40, hh: zoneH, color: "#67e8f9" },
        { id: "not", label: "Not inherited", x: w * 0.46, y: zoneY, ww: w * 0.50, hh: zoneH, color: "#94a3b8" },
      ];
      lastZones = zones;
      const hits = [];
      ...
      drawLabel(ctx, "Trait Tokens sort", w * 0.5, layout.labelY);
  ...
  arena.registerScene(P + "Lab", (api) => {
    ...
    setTick(() => {
      ...
      ctx.fillStyle = "#67e8f9"; ctx.beginPath(); ctx.arc(hx, h * 0.72, 14, 0, Math.PI * 2); ctx.fill();
      const msg = heat > 0.75 ? "Tokens locked" : heat > 0.55 ? "Traits clear" : heat > 0.3 ? "Clues appearing" : "Traits blurry";
  ...
  arena.registerScene(P + "Rule", (api) => {
    ...
    setTick(() => {
      ...
      drawLabel(ctx, "Trait Tokens rule", w * 0.5, layout.labelY);
      failFlash(ctx, w, h); successFlash(ctx, w, h);
    });
    setDispose(() => {});
  });
  ...
  arena.registerScene(P + "Myth", (api) => {
    const { ctx, setTick, setDispose, setDescription, drawBackdrop, setHitRegions, setIntentHandler } = api;
    const myths = [
      { claim: "Everything about you is inherited", truth: "Skills, scars, and choices are not gene traits" },
      { claim: "Traits never come from parents", truth: "Many traits can pass from parents via genes" },
      { claim: "Eye color is only a hat choice", truth: "Eye color is a classic inherited clue" },
      { claim: "A scar is a gene trait", truth: "A scar is from an event - not inherited" },
      { claim: "Only adults have traits", truth: "Kids show inherited traits too" },
    ];
  ...
  arena.registerScene(P + "Drill", (api) => {
    ...
    setTick(() => {
      ...
      drawBackdrop();
      drawLabel(ctx, labState.prompt || "Trait Tokens drill", w * 0.5, h * 0.2, { h: 32, font: "700 16px Segoe UI" });
  ...
  arena.registerScene(P + "Mastery", (api) => {
    ...
    setTick(() => {
      ...
      drawLabel(ctx, "Trait Tokens mastery!", w * 0.5, layout.labelY);
      failFlash(ctx, w, h); successFlash(ctx, w, h);
    });
    setDispose(() => {});
  });
