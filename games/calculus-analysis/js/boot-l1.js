/**
 * Calculus & Analysis boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerSlopeScenes } from "./slope-scenes.js";
...
export async function bootCalculusAnalysis({ manifest }) {

const BOOKS = [BOOK_L1];
let bookApi = { onBookClick() { console.warn("[books] unavailable"); } };
try {
  bookApi = setupMissionBooks({
  subject: manifest.title || "calculus-analysis",
  ...
  await initI18n({ localeStorageKey: manifest.localeKey || "gq-calculus-analysis-locale" });
  ...
  if (coachName) coachName.textContent = manifest.coachName || "Coach Slope";
  ...
  const storageKey = manifest.storageKey || "gq-calculus-analysis-save-v2";

  if (!canvas) throw new Error("Missing #c3d canvas");

  const arena = createArena2D(canvas, { defaultScene: "slopeMeet" });
  window.__arena = arena;
    registerSlopeScenes(arena);
  ...
  const legacy = !saved ? loadSave("gq-calculus-analysis-save-v1") : null;
  ...
  function showHub() {
    ...
    mountMissionHub(hubRoot, {
      ...
      gameTitle: manifest.title || "Calculus & Analysis",
      missions: MISSIONS,
      completed: state.completed,
      forceAllLocked: false,
      unlockByProgress: false,
      subtitle: "Mission 1 is live (Canvas 2D). Slope Stories.",
  ...
  function openIntro(then) {
    ...
    const introScene =
      state.level === 0
        ? ["slopeMeet", { phase: "desk", dwellMs: 3200 }]
        : state.level === 1
          ? ["pushMeet", { phase: "predict", dwellMs: 3200 }]
          : state.level === 2
            ? ["pairMeet", { phase: "desk", dwellMs: 3200 }]
            : ["slopeMeet", { phase: "settle", dwellMs: 2400 }];
  ...
  function runComingSoon() {
    ...
    playScene("slopeMeet", { phase: "settle" });
    overlay.innerHTML = `
      <div class="chem-card">
  ...
  btnHint?.addEventListener("click", () => {
    const hints = [
      [
        "See gentle vs steep lines.",
        "Dial until slope/rate idea is clear.",
        "Sort slope ideas vs not-slope.",
        "Push steeper = faster change.",
        "Order look -> rise/run -> rate -> steep.",
        "Build: Slope = rate of change.",
        "Tap hill and graph contexts.",
        "Bust myths that flat changes fastest.",
        "Hit slope fluency checks.",
        "Claim Slope Scout.",
      ],
      [
        "Coming soon hint.",
  ...
  btnResetAll?.addEventListener("click", () => {
    if (!confirm("Reset all Calculus & Analysis progress?")) return;
    clearSave(storageKey);
    clearSave("gq-calculus-analysis-save-v1");
    state.level = 0;
    state.sub = 0;
  ...
  document.documentElement.classList.add("calculusanal-live", "chem-l1-live");
