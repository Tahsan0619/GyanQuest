/**
 * Slope Stories - Canvas 2D scenes (deepened).
 */
...
function drawLabel(ctx, text, x, y, opts = {}) {
  ...
  ctx.fill();
  ctx.strokeStyle = opts.border || "#c4b5fd";
  ctx.lineWidth = 1.4;
  ctx.stroke();
  ctx.fillStyle = opts.color || "#ede9fe";
...
function drawHero(ctx, w, h, heat, phase) {
  ...
  const y0 = h * 0.62, x0 = w * 0.12;
  // axes
  ctx.strokeStyle = "#64748b"; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(x0, y0); ctx.lineTo(w * 0.9, y0); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(x0, y0); ctx.lineTo(x0, h * 0.18); ctx.stroke();
  const steep = 0.25 + heat * 0.7;
  ctx.strokeStyle = glow ? "#c4b5fd" : "#8b5cf6"; ctx.lineWidth = 4;
  ctx.beginPath();
  ctx.moveTo(x0, y0);
  ctx.lineTo(w * 0.85, y0 - (w * 0.7) * steep * 0.45);
  ctx.stroke();
  ctx.fillStyle = "#e2e8f0"; ctx.font = "700 12px Segoe UI"; ctx.textAlign = "center";
  ctx.fillText(steep > 0.6 ? "Steep = fast change" : "Dial the slope", w * 0.55, h * 0.22);
  ctx.fillText("rise / run", w * 0.55, h * 0.72);
}


export function registerSlopeScenes(arena) {
  ...
  arena.registerScene(P + "Meet", (api) => {
    ...
    setDescription("Slope Stories - meet");
    setTick(() => {
      ...
      const tips = {
        desk: "See gentle, medium, and steep lines.",
        glow: "Steeper line = faster change (rate).",
        settle: "Slope tells the rate - rise over run.",
      };
  ...
  arena.registerScene(P + "Sort", (api) => {
    ...
    const chips = [
      { id: "steep", short: "Steep", color: 0xc4b5fd },
      { id: "gentle", short: "Gentle", color: 0xa78bfa },
      { id: "flat", short: "Flat", color: 0x8b5cf6 },
      { id: "rate", short: "Rate", color: 0xddd6fe },
      { id: "rise", short: "Rise/run", color: 0xfbbf24 },
      { id: "graph", short: "Graph", color: 0x818cf8 },
      { id: "color", short: "Color", color: 0x64748b },
      { id: "song", short: "Song", color: 0x78716c },
    ];
    const accept = {
      slope: ["steep", "gentle", "flat", "rate", "rise", "graph"],
      not: ["color", "song"],
    };
    ...
    setTick(() => {
      ...
      const zones = [
        { id: "slope", label: "Slope idea", x: w * 0.02, y: zoneY, ww: w * 0.48, hh: zoneH, color: "#c4b5fd" },
        { id: "not", label: "Not slope", x: w * 0.54, y: zoneY, ww: w * 0.44, hh: zoneH, color: "#94a3b8" },
      ];
      lastZones = zones;
      const hits = [];
      ...
      drawLabel(ctx, "Slope Stories sort", w * 0.5, layout.labelY);
  ...
  arena.registerScene(P + "Lab", (api) => {
    ...
    setTick(() => {
      ...
      ctx.fillStyle = "#c4b5fd"; ctx.beginPath(); ctx.arc(hx, h * 0.72, 14, 0, Math.PI * 2); ctx.fill();
      const msg = heat > 0.75 ? "Rate locked" : heat > 0.55 ? "Slope clear" : heat > 0.3 ? "Rate forming" : "Slope blurry";
  ...
  arena.registerScene(P + "Rule", (api) => {
    ...
    setTick(() => {
      ...
      drawLabel(ctx, "Slope Stories rule", w * 0.5, layout.labelY);
      failFlash(ctx, w, h); successFlash(ctx, w, h);
    });
    setDispose(() => {});
  });
  ...
  arena.registerScene(P + "Myth", (api) => {
    const { ctx, setTick, setDispose, setDescription, drawBackdrop, setHitRegions, setIntentHandler } = api;
    const myths = [
      { claim: "Slope only means a hill outdoors", truth: "Slope also means rate of change on a graph" },
      { claim: "Flat lines change the fastest", truth: "Flat means little or no change" },
      { claim: "Steep always means slow", truth: "Steep usually means faster change" },
      { claim: "Rise over run is useless", truth: "Rise/run is the kid-friendly slope recipe" },
      { claim: "Songs measure slope", truth: "Graphs and rates measure slope - not songs" },
    ];
  ...
  arena.registerScene(P + "Drill", (api) => {
    ...
    setTick(() => {
      ...
      drawBackdrop();
      drawLabel(ctx, labState.prompt || "Slope Stories drill", w * 0.5, h * 0.2, { h: 32, font: "700 16px Segoe UI" });
  ...
  arena.registerScene(P + "Mastery", (api) => {
    ...
    setTick(() => {
      ...
      drawLabel(ctx, "Slope Stories mastery!", w * 0.5, layout.labelY);
      failFlash(ctx, w, h); successFlash(ctx, w, h);
    });
    setDispose(() => {});
  });
