...
export function mountSpeedDrill(host, cfg) {
  ...
  function render() {
    if (idx >= items.length) {
      ...
      playScene(cfg.passScene || cfg.scene || "slopeMastery");
      host.innerHTML = `
        <div class="chem-card">
    ...
    playScene(cfg.scene || "slopeDrill", { prompt: chemLabState.prompt, flashColor: chemLabState.flashColor });
    host.innerHTML = `
...
export function mountMythCards(host, cfg) {
  ...
  function show() {
    ...
    playScene(cfg.scene || "slopeMyth", { myth: m.sceneMyth });
    host.innerHTML = `
