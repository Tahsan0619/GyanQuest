...
export function mountHeatLab(host, cfg) {
 ...
 // Defaults keep What is AI framing; Mission 1/2 pass topic overrides.
 const sliderLabel = cfg.sliderLabel || "Pattern clarity / guess confidence";
 ...
 ${badgeHtml(cfg.badge || ATOM_ASSET_PATHS.m1, "ai lab")}
 <h3>${cfg.title}</h3>
 <p>${cfg.html}</p>
...
export function mountScaleLab(host, cfg) {
 ...
 // Defaults keep What is AI framing; Mission 1 passes topic overrides.
 const sliderLabel = cfg.sliderLabel || "AI scale: tools → patterns → rule";
 const goalText = cfg.goalText || "Left canvas: everyday tools → pattern cloud → AI rule.";
...
export function mountSpeedDrill(host, cfg) {
 ...
 playScene(cfg.passScene || cfg.scene || "aiMastery");
 host.innerHTML = `
 <div class="chem-card">
 ...
 playScene(cfg.scene || "aiDrill", { prompt: chemLabState.prompt, flashColor: chemLabState.flashColor });
 host.innerHTML = `
...
export function mountMythCards(host, cfg) {
 ...
 playScene(cfg.scene || "aiMyth", { myth: m.sceneMyth });
 host.innerHTML = `
