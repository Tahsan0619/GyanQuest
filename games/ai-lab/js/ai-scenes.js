/**
 * Artificial Intelligence - Mission 1: What is AI? (deepened Canvas 2D).
 */
...
function drawHero(ctx, x, y, heat) {
  ...
  ctx.beginPath(); ctx.arc(x, y, 50 + h * 20, 0, Math.PI * 2); ctx.fill();
  // brain-ish nodes
  const nodes = [[-30,-10],[0,-28],[30,-8],[-18,18],[18,20],[0,4]];
  ctx.strokeStyle = "#c084fc"; ctx.lineWidth = 2;
  ctx.beginPath();
  nodes.forEach((n, i) => { if (i === 0) ctx.moveTo(x+n[0], y+n[1]); else ctx.lineTo(x+n[0], y+n[1]); });
  ctx.closePath(); ctx.stroke();
  nodes.forEach((n) => {
    ctx.fillStyle = "#e9d5ff";
    ctx.beginPath(); ctx.arc(x+n[0], y+n[1], 6, 0, Math.PI * 2); ctx.fill();
  });
  ctx.fillStyle = "#e9d5ff"; ctx.font = "700 12px Segoe UI"; ctx.textAlign = "center";
  ctx.fillText("AI patterns", x, y + 58);
}
...
export function registerAiScenes(arena) {
  ...
  arena.registerScene("aiMeet", (api) => {
    ...
    setDescription("What is AI? - meet the idea");
    setTick(() => {
  ...
  arena.registerScene("aiSort", (api) => {
    ...
    setTick(() => {
      ...
      const zones = [
        { id: "ai", label: "AI tool", x: w * 0.02, y: zoneY, ww: w * 0.3, hh: zoneH, color: "#c084fc" },
  ...
  arena.registerScene("aiLab", (api) => {
    ...
    setDescription("What is AI? lab dial");
    setIntentHandler((intent) => {
  ...
  arena.registerScene("aiRule", (api) => {
    ...
    setDescription("What is AI? rule");
    ...
    setTick(() => {
      ...
      drawLabel(ctx, "What is AI? rule", w * 0.5, layout.labelY);
      failFlash(ctx, w, h); successFlash(ctx, w, h);
    });
    setDispose(() => {});
  });
  ...
  arena.registerScene("aiStretch", (api) => {
    ...
    setTick(() => {
      ...
      const captions = {
        home: "Phone photo tags",
        school: "Voice assistant guesses",
        street: "Map route suggestions",
  ...
  arena.registerScene("aiMyth", (api) => {
    const { ctx, setTick, setDispose, setDescription, drawBackdrop, setHitRegions, setIntentHandler } = api;
    const myths = [
      { claim: "AI is a magic brain that thinks like humans", truth: "AI spots statistical patterns in examples" },
      { claim: "AI never needs data", truth: "Good examples are the fuel for pattern learning" },
      { claim: "One wrong guess means AI is useless", truth: "Guesses improve with better data and checks" },
      { claim: "Every automated button is AI", truth: "Simple switches and fixed scripts are not AI" },
      { claim: "Only adults can understand AI", truth: "Kids can learn: examples -> patterns -> guesses" }
    ];
  ...
  arena.registerScene("aiDrill", (api) => {
    ...
    setTick(() => {
      ...
      drawBackdrop();
      drawLabel(ctx, labState.prompt || "What is AI? drill", w * 0.5, h * 0.2, { h: 32, font: "700 16px Segoe UI" });
  ...
  arena.registerScene("aiMastery", (api) => {
    const { ctx, setTick, setDispose, setDescription, drawBackdrop } = api;
    setDescription("What is AI? mastery.");
    ...
    setTick(() => {
      ...
      drawLabel(ctx, "What is AI?!", w * 0.5, layout.labelY);
      failFlash(ctx, w, h); successFlash(ctx, w, h);
    });
    setDispose(() => {});
  });
