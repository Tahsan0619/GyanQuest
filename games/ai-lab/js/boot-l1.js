/**
 * Artificial Intelligence boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerAiScenes } from "./ai-scenes.js";
...
try {
  ...
  await initI18n({ localeStorageKey: manifest.localeKey || "gq-ai-lab-locale" });
  ...
  if (coachName) coachName.textContent = manifest.coachName || "Coach AI";
  ...
  const storageKey = manifest.storageKey || "gq-ai-lab-save-v2";
  ...
  const legacy = !saved ? loadSave("gq-ai-lab-save-v1") : null;
  ...
  function showHub() {
    ...
    mountMissionHub(hubRoot, {
      ...
      gameTitle: manifest.title || "Artificial Intelligence",
      missions: MISSIONS,
      completed: state.completed,
      forceAllLocked: false,
      unlockByProgress: false,
      subtitle: "Missions 1-2 are live (Canvas 2D). What is AI? | Pattern Predict.",
  ...
  function finishMission() {
    ...
    showNext(false);
    clearOverlay();
    const winScene = state.level === 1 ? "predictMastery" : "aiMastery";
    playScene(winScene);
    if (overlay) {
      overlay.innerHTML = `
        <div class="chem-card">
  ...
  btnHint?.addEventListener("click", () => {
    const hints = [
      ...
      [
        ...
        "Dial prediction confidence.",
        "Sort pattern clues / noise / tricky.",
        "Push confidence higher.",
        "Order: see -> find -> predict -> check.",
        "Build: see pattern -> predict next.",
        "Stretch the idea across places.",
        "Bust predict myths.",
        "Fluency drill.",
        "Claim Pattern Pro.",
      ],
  ...
  btnResetAll?.addEventListener("click", () => {
    if (!confirm("Reset all Artificial Intelligence progress?")) return;
    clearSave(storageKey);
    clearSave("gq-ai-lab-save-v1");
    state.level = 0;
    state.sub = 0;
  ...
  document.documentElement.classList.add("ailab-live", "chem-l1-live");
