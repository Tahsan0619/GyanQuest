/**
 * Artificial Intelligence - Mission 2: Pattern Predict (deepened Canvas 2D).
 */
...
function drawLabel(ctx, text, x, y, opts = {}) {
 ...
 ctx.fill();
 ctx.strokeStyle = opts.border || "rgba(192,132,252,0.55)";
 ctx.lineWidth = 1.4;
 ctx.stroke();
 ctx.fillStyle = opts.color || "#e9d5ff";
...
function drawHero(ctx, x, y, heat) {
 const h = heat ?? 0.5;
 const cols = ["#c084fc", "#a78bfa", "#38bdf8", "#22c55e"];
 const n = 4 + Math.floor(h * 2);
 for (let i = 0; i < n; i++) {
 ctx.fillStyle = cols[i % cols.length];
 roundRect(ctx, x - 70 + i * 36, y - 20, 28, 40, 6); ctx.fill();
 }
 ctx.fillStyle = "rgba(192,132,252,0.35)";
 roundRect(ctx, x - 70 + n * 36, y - 20, 28, 40, 6); ctx.fill();
 ctx.strokeStyle = "#e9d5ff"; ctx.setLineDash([4, 4]); ctx.stroke(); ctx.setLineDash([]);
 ctx.fillStyle = "#e9d5ff"; ctx.font = "700 12px Segoe UI"; ctx.textAlign = "center";
 ctx.fillText("Predict next", x, y + 48);
}

export function registerPredictScenes(arena) {
 if (!arena?.registerScene) return;

 arena.registerScene("predictMeet", (api) => {
 ...
 setDescription("Pattern Predict - meet the idea");
 setTick(() => {
 ...
 arena.registerScene("predictSort", (api) => {
 const { ctx, setTick, setDispose, setDescription, drawBackdrop, setHitRegions, setIntentHandler, reducedMotion } = api;
 setDescription("Drag into Pattern clue / Noise / Tricky.");
 const chips = [
 { id: "rep", short: "Repeat", color: 12616956 },
 { id: "gap", short: "Gap", color: 10980346 },
 { id: "next", short: "Next", color: 3718648 },
 { id: "blot", short: "Blot", color: 9741240 },
 { id: "once", short: "Once", color: 7893356 },
 { id: "twist", short: "Twist", color: 16347926 },
 { id: "beat", short: "Beat", color: 2278750 },
 { id: "noise", short: "Noise", color: 6583435 }
 ];
 const accept = {
 clue: ["rep", "gap", "next", "beat"],
 noise: ["blot", "once", "noise"],
 tricky: ["twist"]
 };
 ...
 const zones = [
 { id: "clue", label: "Pattern clue", x: w * 0.02, y: zoneY, ww: w * 0.3, hh: zoneH, color: "#c084fc" },
 { id: "noise", label: "Noise", x: w * 0.35, y: zoneY, ww: w * 0.3, hh: zoneH, color: "#94a3b8" },
 ...
 const byZone = { clue: [], noise: [], tricky: [] };
 ...
 ctx.fillStyle = "#e9d5ff"; ctx.font = "700 11px Segoe UI"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
 ...
 arena.registerScene("predictRule", (api) => {
 const { ctx, setTick, setDispose, setDescription, drawBackdrop } = api;
 setDescription("Pattern Predict rule");
 const tokens = ["See", "pattern", "->", "predict next"];
 setTick(() => {
 ...
 ctx.fillStyle = "#e9d5ff"; ctx.font = "700 11px Segoe UI"; ctx.textAlign = "center"; ctx.fillText(label, x, h * 0.36);
 });
 drawHero(ctx, w * 0.5, h * 0.58, Math.min(1, prog / 4));
 drawLabel(ctx, "Pattern Predict rule", w * 0.5, layout.labelY);
 failFlash(ctx, w, h); successFlash(ctx, w, h);
 });
 setDispose(() => {});
 });

 arena.registerScene("predictStretch", (api) => {
 const { ctx, setTick, setDispose, setDescription, drawBackdrop, setHitRegions, setIntentHandler } = api;
 ...
 ctx.fillStyle = "#e9d5ff"; ctx.font = "600 11px Segoe UI"; ctx.textAlign = "center"; ctx.fillText(m, x, layout.deskTop - 10);
 hits.push({ id: m, shape: "rect", x, y: layout.deskTop - 12, w: 72, h: 48, meta: { mode: m } });
 });
 drawHero(ctx, w * 0.5, h * 0.4, 0.85);
 const captions = {
 home: "Bead necklace colors",
 school: "Traffic light order",
 street: "Class timetable blocks",
 ...
 arena.registerScene("predictMyth", (api) => {
 const { ctx, setTick, setDispose, setDescription, drawBackdrop, setHitRegions, setIntentHandler } = api;
 const myths = [
 { claim: "Predicting means you never need a pattern", truth: "Predictions ride on the repeating structure you noticed" },
 { claim: "One lucky guess equals mastery", truth: "Check against the real next piece" },
 { claim: "Noise is the same as a pattern clue", truth: "Noise does not reliably repeat" },
 { claim: "Only computers can predict sequences", truth: "Kids predict bead and light patterns every day" },
 { claim: "If you miss once, stop forever", truth: "Misses teach you to re-check the pattern" }
 ];
 ...
 arena.registerScene("predictDrill", (api) => {
 const { ctx, setTick, setDispose, setDescription, drawBackdrop } = api;
 setDescription(labState.prompt || "Pattern Predict drill");
 setTick(() => {
 const w = api.width, h = api.height;
 drawBackdrop();
 drawLabel(ctx, labState.prompt || "Pattern Predict drill", w * 0.5, h * 0.2, { h: 32, font: "700 16px Segoe UI" });
 ...
 arena.registerScene("predictMastery", (api) => {
 const { ctx, setTick, setDispose, setDescription, drawBackdrop } = api;
 setDescription("Pattern Predict mastery.");
 const labels = ["Meet", "Sort", "Lab", "Rule", "Myth", "Win"];
 setTick(() => {
 const w = api.width, h = api.height, layout = api.layout, locked = labState.masteryStep || 0;
 drawBackdrop();
 labels.forEach((label, i) => {
 const x = w * 0.1 + i * (w * 0.14);
 ctx.fillStyle = i < locked ? "#e9d5ff" : "rgba(148,163,184,0.35)";
 ...
 drawLabel(ctx, "Pattern Predict!", w * 0.5, layout.labelY);
 failFlash(ctx, w, h); successFlash(ctx, w, h);
 });
 setDispose(() => {});
 });
