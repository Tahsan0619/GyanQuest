/**
 * Mechanical Basics boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerLeverScenes } from "./lever-scenes.js";
...
try {
  ...
  await initI18n({ localeStorageKey: manifest.localeKey || "gq-mechanical-basics-locale" });
  ...
  if (coachName) coachName.textContent = manifest.coachName || "Coach Gear";
  ...
  const storageKey = manifest.storageKey || "gq-mechanical-basics-save-v2";
  ...
  const legacy = !saved ? loadSave("gq-mechanical-basics-save-v1") : null;
  ...
  function showHub() {
    ...
    mountMissionHub(hubRoot, {
      ...
      gameTitle: manifest.title || "Mechanical Basics",
      missions: MISSIONS,
  ...
  function finishMission() {
    ...
    showNext(false);
    clearOverlay();
    const winScene = state.level === 0 ? "leverMastery" : state.level === 1 ? "motionMastery" : "workMastery";
    playScene(winScene);
    if (overlay) {
      overlay.innerHTML = `
        <div class="chem-card">
  ...
  function runComingSoon() {
    ...
    overlay.innerHTML = `
      <div class="chem-card">
        ...
        <p>${meta.hook}</p>
        <p><strong>Coming soon.</strong> Full interactive labs will ship mission by mission.</p>
  ...
  btnHint?.addEventListener("click", () => {
    const hints = [
      [
        "Drag the lever and meshing gears - find fulcrum, load, effort.",
        "Dial mechanical advantage until the load lifts easier.",
        "Sort levers, gears, and neither.",
        "Push advantage higher - watch gears and lever respond.",
        "Order why the lever helps, then check.",
        "Build: Lever trades force distance.",
        "Tap home / school / street / shop / lab contexts.",
        "Bust lever and gear myths.",
        "Hit the fluency checks.",
        "Order your journey and claim Lever Learner.",
      ],
      [
        "Drag two wheels and find the belt link.",
        "Tighten the belt until transfer is strong.",
        "Sort transfer, block, and neither.",
        "Push transfer higher - both wheels spin.",
        "Order how motion passes along the path.",
        "Build: Linked parts transfer motion.",
        "Tap fan, kit, rickshaw, conveyor, lab.",
        "Bust motion-transfer myths.",
        "Hit the fluency checks.",
        "Claim Motion Scout.",
      ],
      [
        "Watch force move a crate through a distance.",
        "Dial force x distance until work grows.",
        "Sort work, force-only, and neither.",
        "Push work higher - crate travels farther.",
        "Order why work happens.",
        "Build: Work = force x distance.",
        "Tap bags, desks, rickshaw, shop, lab.",
        "Bust work myths.",
        "Hit the fluency checks.",
        "Claim Work Warrior.",
      ],
  ...
  btnResetAll?.addEventListener("click", () => {
    if (!confirm("Reset all Mechanical Basics progress?")) return;
    clearSave(storageKey);
    clearSave("gq-mechanical-basics-save-v1");
    state.level = 0;
    state.sub = 0;
  ...
  document.documentElement.classList.add("mechanicalba-live", "chem-l1-live");
