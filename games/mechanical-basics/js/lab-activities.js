...
export function mountHeatLab(host, cfg) {
 ...
 // Defaults keep Levers & Gears framing; Mission labs pass topic overrides.
 const sliderLabel = cfg.sliderLabel || "Mechanical advantage";
 ...
 ${badgeHtml(cfg.badge || ATOM_ASSET_PATHS.m1, "lever lab")}
 <h3>${cfg.title}</h3>
 <p>${cfg.html}</p>
...
export function mountScaleLab(host, cfg) {
 ...
 // Defaults keep Levers & Gears framing; Mission 1 passes topic overrides.
 const sliderLabel = cfg.sliderLabel || "Machine scale: desk tools \u2192 arms labeled \u2192 RULE";
 const goalText = cfg.goalText || "Left canvas follows: everyday lever/gears \u2192 labeled parts \u2192 MACHINE RULE.";
...
export function mountSpeedDrill(host, cfg) {
 ...
 playScene(cfg.passScene || cfg.scene || "leverMastery");
 host.innerHTML = `
 <div class="chem-card">
 ...
 playScene(cfg.scene || "leverDrill", { prompt: chemLabState.prompt, flashColor: chemLabState.flashColor });
 host.innerHTML = `
...
export function mountMythCards(host, cfg) {
 ...
 playScene(cfg.scene || "leverMyth", { myth: m.sceneMyth });
 host.innerHTML = `
...
export function mountSpiralMap(host, cfg) {
 ...
 <button type="button" class="btn primary tiny-pulse" id="spiral-go">${cfg.finishLabel || "Finish Levers & Gears ▶"}</button>
 </div>`;
 function playStop(n) {
 if (cancelled) return;
 chemLabState.spiralStop = n;
 chemLabState.spiralUntil = performance.now() + 4500;
 playScene(cfg.scene || "levSpiral", { spiralStop: n });
 }
