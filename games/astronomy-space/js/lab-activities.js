...
export function mountHeatLab(host, cfg) {
 ...
 // Astronomy defaults (not chemistry melt / force lab framing).
 const sliderLabel = cfg.sliderLabel || "Orbit clarity";
 ...
 ${badgeHtml(cfg.badge || ATOM_ASSET_PATHS.life, "orbit lab")}
 <h3>${cfg.title}</h3>
 <p>${cfg.html}</p>
 ...
 <p class="chem-heat__readout" id="chem-heat-read" aria-live="polite">Blurry</p>
 ...
 cold: "Blurry - hard to see orbit paths",
...
export function mountScaleLab(host, cfg) {
 ...
 // Solar Family defaults (not Tiny Bits salt grain → ions → orbitals).
 const sliderLabel = cfg.sliderLabel || "Orbit scale: desk \u2192 paths \u2192 rule";
 const goalText = cfg.goalText || "Left canvas follows: desk family \u2192 glowing orbits \u2192 PLANETS ORBIT THE SUN.";
...
export function mountSpeedDrill(host, cfg) {
 ...
 playScene(cfg.passScene || cfg.scene || "solarMastery");
 host.innerHTML = `
 <div class="chem-card">
 ...
 playScene(cfg.scene || "solarDrill", { prompt: chemLabState.prompt, flashColor: chemLabState.flashColor });
 host.innerHTML = `
...
export function mountMythCards(host, cfg) {
 ...
 playScene(cfg.scene || "solarMyth", { myth: m.sceneMyth });
 host.innerHTML = `
...
export function mountSpiralMap(host, cfg) {
 ...
 playScene(cfg.scene || "solarSpiral");
 ...
 <button type="button" class="btn primary tiny-pulse" id="spiral-go">${cfg.finishLabel || "Finish Solar Family ▶"}</button>
 </div>`;
 function playStop(n) {
 if (cancelled) return;
 chemLabState.spiralStop = n;
 chemLabState.spiralUntil = performance.now() + 4500;
 playScene(cfg.scene || "solarSpiral", { spiralStop: n });
 }
