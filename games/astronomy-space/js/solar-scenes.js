/**
 * Solar Family - Canvas 2D scenes (deepened).
 */
...
function drawHero(ctx, w, h, heat, phase) {
  ...
  ctx.fillStyle = "#78350f"; ctx.font = "700 11px Segoe UI"; ctx.textAlign = "center";
  ctx.fillText("Sun", cx, cy + 4);
  // Orbit rings + planets
  const orbits = [
    { r: 55 + heat * 8, col: "#94a3b8", name: "Merc", ang: 0.2 },
    { r: 78 + heat * 10, col: "#38bdf8", name: "Earth", ang: 1.2 },
    { r: 105 + heat * 12, col: "#fbbf24", name: "Jup", ang: 2.4 },
  ];
  orbits.forEach((o) => {
    ctx.strokeStyle = "rgba(129,140,248,0.35)"; ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.arc(cx, cy, o.r, 0, Math.PI * 2); ctx.stroke();
    const px = cx + Math.cos(o.ang + heat * 2) * o.r;
    const py = cy + Math.sin(o.ang + heat * 2) * o.r;
    ctx.fillStyle = o.col; ctx.beginPath(); ctx.arc(px, py, 7, 0, Math.PI * 2); ctx.fill();
    if (glow) {
      ctx.fillStyle = "#e2e8f0"; ctx.font = "600 10px Segoe UI";
...
export function registerSolarScenes(arena) {
  ...
  arena.registerScene(P + "Sort", (api) => {
    ...
    setTick(() => {
      ...
      const zones = [
        ...
        { id: "other", label: "Moon/comet", x: w * 0.50, y: zoneY, ww: w * 0.23, hh: zoneH, color: "#a78bfa" },
  ...
  arena.registerScene(P + "Lab", (api) => {
    ...
    setTick(() => {
      ...
      const msg = heat > 0.75 ? "Family locked" : heat > 0.55 ? "Orbits clear" : heat > 0.3 ? "Paths appearing" : "Orbit blurry";
  ...
  arena.registerScene(P + "Rule", (api) => {
    ...
    setTick(() => {
      ...
      drawLabel(ctx, "Solar Family rule", w * 0.5, layout.labelY);
      failFlash(ctx, w, h); successFlash(ctx, w, h);
    });
    setDispose(() => {});
  });
  ...
  arena.registerScene(P + "Drill", (api) => {
    ...
    setTick(() => {
      ...
      drawBackdrop();
      drawLabel(ctx, labState.prompt || "Solar Family drill", w * 0.5, h * 0.2, { h: 32, font: "700 16px Segoe UI" });
