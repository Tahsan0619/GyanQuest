/**
 * Astronomy & Space Canvas 2D arena - layout-aware, hit-testable, resize-safe.
 */
...
export function createArena2D(canvas, theme = {}) {
 ...
 let lastSceneName = theme.defaultScene || "solarMeet";
 let hitRegions = [];
 let dragState = null;
 ...
 // Deep space desk (not chemistry green / bio stage).
 ...
 const rg = ctx.createRadialGradient(w * 0.5, h * 0.22, 10, w * 0.5, h * 0.42, w * 0.72);
 rg.addColorStop(0, "rgba(250,204,21,0.12)");
 rg.addColorStop(0.45, "rgba(129,140,248,0.08)");
 ...
 const sy = ((i * 53) % 55) / 100;
 ctx.beginPath();
 ctx.arc(w * sx, h * sy * 0.55, i % 5 === 0 ? 1.6 : 1, 0, Math.PI * 2);
 ctx.fill();
 }

 // Floor / observatory ledge
 ...
 const description = text || "Solar Family canvas stage";
 ...
 clearScene();
 const fn = SCENES[name] || SCENES.solarMeet;
 if (fn) fn();
 ...
 playExample("solarMeet", { phase: "desk" });
 }
