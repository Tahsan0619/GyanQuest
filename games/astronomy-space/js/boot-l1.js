/**
 * Astronomy & Space boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { createArena2D } from "./arena-2d.js?v=solar4";
import { playScene, cancelActiveActivity } from "./lab-activities.js?v=solar4";
import { runL1Sub, L1_META } from "./level1.js?v=solar4";
...
import { registerSolarScenes } from "./solar-scenes.js?v=solar4";
import { registerSkyScenes } from "./sky-scenes.js";
import { BOOK as BOOK_L1 } from "../books/level1.js?v=genas1";
...
try {
 ...
 await initI18n({ localeStorageKey: manifest.localeKey || "gq-astronomy-space-locale" });
 ...
 if (coachName) coachName.textContent = manifest.coachName || "Coach Orbit";
 ...
 const storageKey = manifest.storageKey || "gq-astronomy-space-save-v2";

 if (!canvas) throw new Error("Missing #c3d canvas");

 const arena = createArena2D(canvas, { defaultScene: "solarOpen" });
 window.__arena = arena;
 registerSolarScenes(arena);
 registerSkyScenes(arena);
 ...
 const legacy = !saved ? loadSave("gq-astronomy-space-save-v1") : null;
 ...
 gameTitle: manifest.title || "Astronomy & Space",
 missions: MISSIONS,
 completed: state.completed,
 forceAllLocked: false,
 unlockByProgress: false,
 subtitle: "Missions 1-2 are live (Canvas 2D). Solar Family / Day & Night Sky.",
 ...
 playScene("solarOpen", { dwellMs: 2400 });
 overlay.innerHTML = `
 <div class="chem-card">
 ...
 const hints = [
 [
 "Drag Sun and planets - see the family.",
 "Dial until orbit paths look clear.",
 "Sort planet / Sun / moon-comet / not-space.",
 "Push orbit clarity higher.",
 "Order why planets orbit the Sun.",
 "Build: Planets orbit the Sun.",
 "Tap home / school / street / bd / lab sky contexts.",
 "Bust myths about Sun orbiting Earth.",
 "Hit the orbit fluency checks.",
 "Order the journey and claim Orbit Scout.",
 ],
 [
 "See bright side vs shadow side on Earth.",
 "Dial until spin idea is clear.",
 "Sort cause / result / not-the-cause.",
 "Push day-night clarity higher.",
 "Order sun -> spin -> side -> day/night.",
 "Build: Earth spins = day/night.",
 "Tap contexts - Dhaka night can be day elsewhere.",
 "Bust myths about the Sun sleeping.",
 "Hit day-night fluency checks.",
 "Claim Sky Watcher.",
 ],
 [
 "Coming soon hint.",
 ...
 if (!confirm("Reset all Astronomy & Space progress?")) return;
 clearSave(storageKey);
 clearSave("gq-astronomy-space-save-v1");
 state.level = 0;
 state.sub = 0;
 ...
 document.documentElement.classList.add("astronomyspa-live", "chem-l1-live");

 // QA jump helper (tools/_qa-jump-astro.html) may set sessionStorage to resume mid-mission.
