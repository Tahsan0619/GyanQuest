/**
 * Electronics & Robotics boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerBotScenes } from "./bot-scenes.js";
...
export async function bootElectronicsRobotics({ manifest }) {

const BOOKS = [BOOK_L1];
let bookApi = { onBookClick() { console.warn("[books] unavailable"); } };
try {
  bookApi = setupMissionBooks({
  subject: manifest.title || "electronics-robotics",
  ...
  await initI18n({ localeStorageKey: manifest.localeKey || "gq-electronics-robotics-locale" });
  ...
  if (coachName) coachName.textContent = manifest.coachName || "Coach Bot";
  ...
  const storageKey = manifest.storageKey || "gq-electronics-robotics-save-v2";

  if (!canvas) throw new Error("Missing #c3d canvas");

  const arena = createArena2D(canvas, { defaultScene: "botMeet" });
  window.__arena = arena;
    registerBotScenes(arena);
  ...
  const legacy = !saved ? loadSave("gq-electronics-robotics-save-v1") : null;
  ...
  function showHub() {
    ...
    mountMissionHub(hubRoot, {
      ...
      gameTitle: manifest.title || "Electronics & Robotics",
      missions: MISSIONS,
      completed: state.completed,
      forceAllLocked: false,
      unlockByProgress: false,
      subtitle: "Mission 1 is live (Canvas 2D). Sensor Bot - sense, decide, act.",
  ...
  function openIntro(then) {
    ...
    const introScene =
      state.level === 0
        ? ["botMeet", { phase: "desk", dwellMs: 3200 }]
        : state.level === 1
          ? ["botMeet", { phase: "desk", dwellMs: 3200 }]
          : state.level === 2
            ? ["botMeet", { phase: "desk", dwellMs: 3200 }]
            : ["botMeet", { phase: "settle", dwellMs: 2400 }];
  ...
  function runComingSoon() {
    ...
    playScene("botMeet", { phase: "settle" });
    overlay.innerHTML = `
      <div class="chem-card">
  ...
  btnHint?.addEventListener("click", () => {
    const hints = [
      [
        "Meet sense, decide, and act on the bot.",
        "Dial the loop until sense-decide-act completes.",
        "Sort sensors, decisions, and actuators.",
        "Push the loop further.",
        "Order why the bot moves.",
        "Build: Sense -> decide -> act.",
        "Tap vacuum, kit, lights, door, lab.",
        "Bust robot myths.",
  ...
  btnResetAll?.addEventListener("click", () => {
    if (!confirm("Reset all Electronics & Robotics progress?")) return;
    clearSave(storageKey);
    clearSave("gq-electronics-robotics-save-v1");
    state.level = 0;
    state.sub = 0;
  ...
  document.documentElement.classList.add("electronicsr-live", "chem-l1-live");
