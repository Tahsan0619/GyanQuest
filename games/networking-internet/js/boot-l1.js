/**
 * Networking & Internet boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerPacketScenes } from "./packet-scenes.js";
...
export async function bootNetworkingInternet({ manifest }) {

const BOOKS = [BOOK_L1];
let bookApi = { onBookClick() { console.warn("[books] unavailable"); } };
try {
  bookApi = setupMissionBooks({
  subject: manifest.title || "networking-internet",
  ...
  await initI18n({ localeStorageKey: manifest.localeKey || "gq-networking-internet-locale" });
  ...
  if (coachName) coachName.textContent = manifest.coachName || "Coach Net";
  ...
  const storageKey = manifest.storageKey || "gq-networking-internet-save-v2";

  if (!canvas) throw new Error("Missing #c3d canvas");

  const arena = createArena2D(canvas, { defaultScene: "packetMeet" });
  window.__arena = arena;
    registerPacketScenes(arena);
  ...
  const legacy = !saved ? loadSave("gq-networking-internet-save-v1") : null;
  ...
  function showHub() {
    ...
    mountMissionHub(hubRoot, {
      ...
      gameTitle: manifest.title || "Networking & Internet",
      missions: MISSIONS,
      completed: state.completed,
      forceAllLocked: false,
      unlockByProgress: false,
      subtitle: "Mission 1 is live (Canvas 2D). Packets Travel.",
  ...
  function openIntro(then) {
    ...
    const introScene =
      state.level === 0
        ? ["packetMeet", { phase: "desk", dwellMs: 3200 }]
        : ["packetMeet", { phase: "settle", dwellMs: 2400 }];
  ...
  function runComingSoon() {
    ...
    playScene("packetMeet", { phase: "settle" });
    overlay.innerHTML = `
      <div class="chem-card">
  ...
  btnResetAll?.addEventListener("click", () => {
    if (!confirm("Reset all Networking & Internet progress?")) return;
    clearSave(storageKey);
    clearSave("gq-networking-internet-save-v1");
    state.level = 0;
    state.sub = 0;
  ...
  document.documentElement.classList.add("networkingin-live", "chem-l1-live");
