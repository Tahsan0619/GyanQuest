/**
 * Eco Guardian activity mounts - panel + canvas share labState / intents.
 */
...
export function mountHeatLab(host, cfg) {
 ...
 // Waste Watch labs pass recycle-fill labels; eco defaults avoid chemistry melt copy.
 const sliderLabel = cfg.sliderLabel || "Recycle fill";
...
export function mountEquationBuild(host, cfg) {
 ...
 if (ok) {
 status.textContent = cfg.lockedText || "Rule locked in. Continue when ready.";
...
export function mountScaleLab(host, cfg) {
 ...
 // Waste Watch passes eco labels; keep chem-style defaults only as last resort.
 const sliderLabel = cfg.sliderLabel || "Priority scale: pile → bins → 3R rule";
 const goalText = cfg.goalText || "Left canvas follows: messy pile → sort bins → Reduce→Reuse→Recycle.";
 const readouts = cfg.readoutLabels || {
 low: "Messy litter pile",
 mid: "Three bins ready to sort",
...
export function mountSpeedDrill(host, cfg) {
 ...
 playScene(cfg.passScene || cfg.scene || "wasteMastery");
 host.innerHTML = `
 <div class="chem-card">
 ...
 playScene(cfg.scene || "wasteDrill", { prompt: chemLabState.prompt, flashColor: chemLabState.flashColor });
 host.innerHTML = `
...
export function mountMythCards(host, cfg) {
 ...
 playScene(cfg.scene || "wasteMyth", { myth: m.sceneMyth });
 host.innerHTML = `
...
export function mountSpiralMap(host, cfg) {
 ...
 playScene(cfg.scene || "wasteSpiral");
 ...
 <button type="button" class="btn primary tiny-pulse" id="spiral-go">${cfg.finishLabel || "Finish Waste Watch ▶"}</button>
 </div>`;
 function playStop(n) {
 if (cancelled) return;
 chemLabState.spiralStop = n;
 chemLabState.spiralUntil = performance.now() + 4500;
 playScene(cfg.scene || "wasteSpiral", { spiralStop: n });
 }
