/**
 * Eco Guardian boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerWasteScenes } from "./waste-scenes.js";
...
export async function bootEcoGuardian({ manifest }) {

const BOOKS = [BOOK_L1];
let bookApi = { onBookClick() { console.warn("[books] unavailable"); } };
try {
  bookApi = setupMissionBooks({
  subject: manifest.title || "eco-guardian",
  ...
  await initI18n({ localeStorageKey: manifest.localeKey || "gq-eco-guardian-locale" });
  ...
  if (coachName) coachName.textContent = manifest.coachName || "Coach Eco";
  ...
  const storageKey = manifest.storageKey || "gq-eco-guardian-save-v2";
  ...
  const legacy = !saved ? loadSave("gq-eco-guardian-save-v1") : null;
  ...
  function showHub() {
    ...
    mountMissionHub(hubRoot, {
      ...
      gameTitle: manifest.title || "Eco Guardian",
      missions: MISSIONS,
      completed: state.completed,
      forceAllLocked: false,
      unlockByProgress: false,
      subtitle: "Missions 1–3 are live (Canvas 2D). Waste Watch.",
  ...
  btnResetAll?.addEventListener("click", () => {
    if (!confirm("Reset all Eco Guardian progress?")) return;
    clearSave(storageKey);
    clearSave("gq-eco-guardian-save-v1");
    state.level = 0;
    state.sub = 0;
  ...
  document.documentElement.classList.add("ecoguardian-live", "chem-l1-live");
