/**
 * Geology & Earth boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
export async function bootGeologyEarth({ manifest }) {

const BOOKS = [BOOK_L1];
let bookApi = { onBookClick() { console.warn("[books] unavailable"); } };
try {
  bookApi = setupMissionBooks({
  subject: manifest.title || "geology-earth",
  ...
  await initI18n({ localeStorageKey: manifest.localeKey || "gq-geology-earth-locale" });
  ...
  if (coachName) coachName.textContent = manifest.coachName || "Coach Rock";
  ...
  const storageKey = manifest.storageKey || "gq-geology-earth-save-v2";
  ...
  const legacy = !saved ? loadSave("gq-geology-earth-save-v1") : null;
  ...
  function showHub() {
    ...
    mountMissionHub(hubRoot, {
      ...
      gameTitle: manifest.title || "Geology & Earth",
      missions: MISSIONS,
      completed: state.completed,
      forceAllLocked: false,
      unlockByProgress: false,
      subtitle: "Mission 1 is live (Canvas 2D). Rock Cycle Lite.",
  ...
  btnHint?.addEventListener("click", () => {
    const hints = [
      [
        "Meet igneous, sedimentary, metamorphic.",
        "Dial until rock-cycle idea is clear.",
        "Sort type / how it forms / not-a-rock.",
        "Push cycle clarity with heat and pressure.",
        "Order melt -> igneous -> wear -> remake.",
        "Build: Rocks can transform types.",
        "Tap pebble / brick / hill contexts.",
        "Bust myths that rocks never change.",
        "Hit rock fluency checks.",
        "Claim Rock Ranger.",
      ],
      [
        "Coming soon hint.",
  ...
  btnResetAll?.addEventListener("click", () => {
    if (!confirm("Reset all Geology & Earth progress?")) return;
    clearSave(storageKey);
    clearSave("gq-geology-earth-save-v1");
    state.level = 0;
    state.sub = 0;
  ...
  document.documentElement.classList.add("geologyearth-live", "chem-l1-live");
