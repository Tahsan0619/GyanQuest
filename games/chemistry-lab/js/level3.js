...
import { BOND_ASSET_PATHS } from "./bond-scenes.js";
import {
  mountMotionChain,
  mountDragSort,
  mountHeatLab,
...
function sub2_attract({ overlay, setCoach, completeSub }) {
  setCoach("Watch: opposite magnets pull together - a kid-friendly model for ionic attraction.");
  ...
  mountMotionChain(overlay, {
    ...
    beats: [
      ...
      },
      {
        scene: "bondMagnet",
        dwellMs: 4800,
        html: `<p><strong>Act 2:</strong> Drag magnets together on the canvas (or use the slider later). N and S attract.</p>
          <p>Opposite charges in chemistry can buddy up in a similar spirit.</p>`,
      },
      {
        scene: "bondMagnet",
        dwellMs: 4000,
        html: `<p><strong>Act 3:</strong> When they almost touch, the bond glow appears - held together.</p>`,
      },
    ],
    onDone: () => {
      mountQuiz(overlay, {
...
function sub6_rule({ overlay, setCoach, completeSub }) {
  setCoach("Symbolic: build the bond rule, then scrub the idea.");
  mountEquationBuild(overlay, {
    ...
    onDone: () => {
      mountScaleLab(overlay, {
        ...
        html: `<p>Scrub from desk magnets -> atom pair -> named bond rule.</p>`,
        threshold: 0.85,
        onDone: () => {
          mountQuiz(overlay, {
            scene: "bondRule",
...
function sub7_stretch({ overlay, setCoach, completeSub }) {
  setCoach("Stretch: salt, O₂, sugar, plastic, protein - same bond idea.");
  const modes = [
  ...
  function step() {
    ...
    mountTapContinue(overlay, {
      ...
      badge: m.badge,
      html: `<h3>${m.title}</h3><p>${m.blurb}</p><p>Tap the desk chips on the canvas, then continue.</p>`,
      onDone: step,
    });
  }
  step();
