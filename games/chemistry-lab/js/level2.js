...
function sub6_rule({ overlay, setCoach, completeSub }) {
  setCoach("Symbolic: build the rule, then scrub the idea from sample -> atoms -> name.");
  mountEquationBuild(overlay, {
    ...
    onDone: () => {
      mountScaleLab(overlay, {
        ...
        html: `<p>Scrub from everyday sample -> atom cloud -> element name.</p>`,
        instructions: "Move the scale scrubber to at least 85%.",
        threshold: 0.85,
        onDone: () => {
          mountQuiz(overlay, {
            scene: "elemRule",
...
function sub7_stretch({ overlay, setCoach, completeSub }) {
  setCoach("Stretch: same rule in gold, foil, charcoal, helium, graphite.");
  const modes = [
    { mode: "gold", title: "Gold ring", blurb: "Jewelry gold is the element Au - one atom kind." },
    { mode: "foil", title: "Aluminum foil", blurb: "Kitchen foil is Al atoms packed flat." },
    { mode: "charcoal", title: "Charcoal", blurb: "Burnt wood charcoal is mostly carbon (C)." },
    { mode: "helium", title: "Helium balloon", blurb: "Party balloons use He atoms - still an element." },
  ...
  function step() {
    ...
    mountTapContinue(overlay, {
      ...
      html: `<h3>${m.title}</h3><p>${m.blurb}</p><p>Tap samples on the canvas, then continue.</p>`,
      onDone: step,
    });
  }
  step();
