/**
 * Machine Learning boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
try {
 ...
 await initI18n({ localeStorageKey: manifest.localeKey || "gq-ml-lab-locale" });
 ...
 if (coachName) coachName.textContent = manifest.coachName || "Coach Model";
 ...
 const storageKey = manifest.storageKey || "gq-ml-lab-save-v2";
 ...
 const legacy = !saved ? loadSave("gq-ml-lab-save-v1") : null;
 ...
 gameTitle: manifest.title || "Machine Learning",
 missions: MISSIONS,
 completed: state.completed,
 forceAllLocked: false,
 unlockByProgress: false,
 subtitle: "Mission 1 is live (Canvas 2D). Teach the Model.",
 ...
 if (!confirm("Reset all Machine Learning progress?")) return;
 clearSave(storageKey);
 clearSave("gq-ml-lab-save-v1");
 state.level = 0;
 state.sub = 0;
