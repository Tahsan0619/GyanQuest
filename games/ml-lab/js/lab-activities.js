...
export function mountHeatLab(host, cfg) {
 ...
 const sliderLabel = cfg.sliderLabel || "Train strength";
...
export function mountSpeedDrill(host, cfg) {
 ...
 playScene(cfg.passScene || cfg.scene || "mlMastery");
 host.innerHTML = `
 <div class="chem-card">
 ...
 playScene(cfg.scene || "mlDrill", { prompt: chemLabState.prompt, flashColor: chemLabState.flashColor });
 host.innerHTML = `
...
export function mountMythCards(host, cfg) {
 ...
 playScene(cfg.scene || "mlMyth", { myth: m.sceneMyth });
 host.innerHTML = `
...
export function mountSpiralMap(host, cfg) {
 ...
 <button type="button" class="btn primary tiny-pulse" id="spiral-go">${cfg.finishLabel || "Finish Teach the Model ▶"}</button>
 </div>`;
 function playStop(n) {
 if (cancelled) return;
 chemLabState.spiralStop = n;
 chemLabState.spiralUntil = performance.now() + 4500;
 playScene(cfg.scene || "mlSpiral", { spiralStop: n });
 }
