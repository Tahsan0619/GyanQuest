/**
 * Machine Learning - Mission 1: Teach the Model (deepened Canvas 2D).
 */
...
function drawLabel(ctx, text, x, y, opts = {}) {
 ...
 ctx.fill();
 ctx.strokeStyle = opts.border || "rgba(244,114,182,0.55)";
 ctx.lineWidth = 1.4;
 ctx.stroke();
...
function drawHero(ctx, x, y, heat) {
 ...
 roundRect(ctx, x - 50, y - 40 + i * 14, 60, 18, 4); ctx.fill();
 }
 ctx.fillStyle = "#1e293b";
 roundRect(ctx, x + 20, y - 30, 50, 70, 8); ctx.fill();
 ctx.strokeStyle = "#f472b6"; ctx.lineWidth = 2; ctx.stroke();
 ctx.fillStyle = "#fce7f3"; ctx.font = "700 11px Segoe UI"; ctx.textAlign = "center";
 ctx.fillText("Model", x + 45, y + 8);
 ctx.fillText("Train", x - 20, y + 58);
}

export function registerMlScenes(arena) {
 if (!arena?.registerScene) return;

 arena.registerScene("mlMeet", (api) => {
 ...
 setDescription("Teach the Model - meet the idea");
 setTick(() => {
 ...
 arena.registerScene("mlSort", (api) => {
 const { ctx, setTick, setDispose, setDescription, drawBackdrop, setHitRegions, setIntentHandler, reducedMotion } = api;
 setDescription("Drag into Train / Test / Junk.");
 const chips = [
 { id: "t1", short: "Train A", color: 16020150 },
 { id: "t2", short: "Train B", color: 16478597 },
 { id: "t3", short: "Train C", color: 2278750 },
 { id: "x1", short: "Test A", color: 3718648 },
 { id: "x2", short: "Test B", color: 959977 },
 { id: "j1", short: "Blank", color: 9741240 },
 { id: "j2", short: "Wrong", color: 16347926 },
 { id: "t4", short: "Train D", color: 10980346 }
 ];
 const accept = {
 train: ["t1", "t2", "t3", "t4"],
 test: ["x1", "x2"],
 ...
 const zones = [
 { id: "train", label: "Train set", x: w * 0.02, y: zoneY, ww: w * 0.3, hh: zoneH, color: "#f472b6" },
 { id: "test", label: "Test set", x: w * 0.35, y: zoneY, ww: w * 0.3, hh: zoneH, color: "#38bdf8" },
 { id: "junk", label: "Junk", x: w * 0.68, y: zoneY, ww: w * 0.3, hh: zoneH, color: "#94a3b8" }
 ];
 lastZones = zones;
 const hits = [];
 ...
 ctx.fillStyle = "#fce7f3"; ctx.font = "700 11px Segoe UI"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
 ...
 arena.registerScene("mlRule", (api) => {
 const { ctx, setTick, setDispose, setDescription, drawBackdrop } = api;
 setDescription("Teach the Model rule");
 const tokens = ["Train", "on examples", "->", "test new"];
 setTick(() => {
 ...
 ctx.fillStyle = "#fce7f3"; ctx.font = "700 11px Segoe UI"; ctx.textAlign = "center"; ctx.fillText(label, x, h * 0.36);
 });
 drawHero(ctx, w * 0.5, h * 0.58, Math.min(1, prog / 4));
 drawLabel(ctx, "Teach the Model rule", w * 0.5, layout.labelY);
 failFlash(ctx, w, h); successFlash(ctx, w, h);
 });
 setDispose(() => {});
 });

 arena.registerScene("mlStretch", (api) => {
 const { ctx, setTick, setDispose, setDescription, drawBackdrop, setHitRegions, setIntentHandler } = api;
 ...
 ctx.fillStyle = "#fce7f3"; ctx.font = "600 11px Segoe UI"; ctx.textAlign = "center"; ctx.fillText(m, x, layout.deskTop - 10);
 hits.push({ id: m, shape: "rect", x, y: layout.deskTop - 12, w: 72, h: 48, meta: { mode: m } });
 });
 drawHero(ctx, w * 0.5, h * 0.4, 0.85);
 const captions = {
 home: "Sorting fruit photos",
 school: "Handwriting samples",
 street: "Spam vs real mail",
 ...
 arena.registerScene("mlMyth", (api) => {
 const { ctx, setTick, setDispose, setDescription, drawBackdrop, setHitRegions, setIntentHandler } = api;
 const myths = [
 { claim: "More random junk always helps", truth: "Quality labeled examples matter more than junk" },
 { claim: "Testing on the train set is enough", truth: "Hold out fresh examples to check for real" },
 { claim: "Models never need updates", truth: "Better data and checks improve them" },
 { claim: "One example teaches everything", truth: "Many varied examples usually teach better" },
 { claim: "Kids cannot teach a simple model idea", truth: "Train vs test is a kid-friendly rule" }
 ];
 ...
 arena.registerScene("mlDrill", (api) => {
 const { ctx, setTick, setDispose, setDescription, drawBackdrop } = api;
 setDescription(labState.prompt || "Teach the Model drill");
 setTick(() => {
 const w = api.width, h = api.height;
 drawBackdrop();
 drawLabel(ctx, labState.prompt || "Teach the Model drill", w * 0.5, h * 0.2, { h: 32, font: "700 16px Segoe UI" });
 ...
 arena.registerScene("mlMastery", (api) => {
 const { ctx, setTick, setDispose, setDescription, drawBackdrop } = api;
 setDescription("Teach the Model mastery.");
 const labels = ["Meet", "Sort", "Lab", "Rule", "Myth", "Ml"];
 setTick(() => {
 const w = api.width, h = api.height, layout = api.layout, locked = labState.masteryStep || 0;
 drawBackdrop();
 labels.forEach((label, i) => {
 const x = w * 0.1 + i * (w * 0.14);
 ctx.fillStyle = i < locked ? "#fce7f3" : "rgba(148,163,184,0.35)";
 ...
 drawLabel(ctx, "Teach the Model!", w * 0.5, layout.labelY);
 failFlash(ctx, w, h); successFlash(ctx, w, h);
 });
 setDispose(() => {});
 });
