/**
 * Geometry & Trigonometry boot - mission hub + Canvas 2D missions (Chem pattern).
 */
...
import { registerShapeScenes } from "./shape-scenes.js";
import { registerAngleScenes } from "./angle-scenes.js";
...
try {
  ...
  await initI18n({ localeStorageKey: manifest.localeKey || "gq-geometry-trig-locale" });
  ...
  if (coachName) coachName.textContent = manifest.coachName || "Coach Angle";
  ...
  const storageKey = manifest.storageKey || "gq-geometry-trig-save-v2";

  if (!canvas) throw new Error("Missing #c3d canvas");

  const arena = createArena2D(canvas, { defaultScene: "shapeMeet" });
  window.__arena = arena;
    registerShapeScenes(arena);
  registerAngleScenes(arena);
  ...
  const legacy = !saved ? loadSave("gq-geometry-trig-save-v1") : null;
  ...
  function showHub() {
    ...
    mountMissionHub(hubRoot, {
      ...
      gameTitle: manifest.title || "Geometry & Trigonometry",
      missions: MISSIONS,
      completed: state.completed,
      forceAllLocked: false,
      unlockByProgress: false,
      subtitle: "Missions 1–3 are live (Canvas 2D). Shape Studio · Angle Adventures.",
  ...
  function openIntro(then) {
    ...
    const introScene =
      state.level === 0
        ? ["shapeMeet", { phase: "desk", dwellMs: 3200 }]
        : state.level === 1
          ? ["pushMeet", { phase: "predict", dwellMs: 3200 }]
          : state.level === 2
            ? ["pairMeet", { phase: "desk", dwellMs: 3200 }]
            : ["shapeMeet", { phase: "settle", dwellMs: 2400 }];
  ...
  function runComingSoon() {
    ...
    playScene("shapeMeet", { phase: "settle" });
    overlay.innerHTML = `
      <div class="chem-card">
  ...
  btnResetAll?.addEventListener("click", () => {
    if (!confirm("Reset all Geometry & Trigonometry progress?")) return;
    clearSave(storageKey);
    clearSave("gq-geometry-trig-save-v1");
    state.level = 0;
    state.sub = 0;
  ...
  document.documentElement.classList.add("geometrytrig-live", "chem-l1-live");
