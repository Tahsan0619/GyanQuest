...
import { detectGameSlug, missionContentTier } from "/engine/js/mission-status.js?v=tier3";

const HUB_CSS_HREF = "/engine/css/mission-hub.css?v=tier3";
const HUB_FONT_HREF =
...
export function mountMissionHub(host, opts) {
 ...
 onBookClick,
 rewards,
 gameSlug: gameSlugOpt,
 } = opts;
 const gameSlug = gameSlugOpt || detectGameSlug();
 ...
 const contentTier = missionContentTier(gameSlug, i, !!m.playable && !forceAllLocked);

 let badge = contentTier.label;
 let kind = contentTier.id === "playable" ? "live" : contentTier.id === "dev" ? "dev" : "soon";
 if (forceAllLocked) {
 badge = "Locked";
 kind = "locked";
 } else if (!playable) {
 ...
 card.dataset.contentTier = contentTier.id;
 ...
 <span class="gq-hub-card__badge gq-hub-card__badge--${contentTier.id}">${escapeHtml(badge)}</span>`;
 playBtn.onclick = () => {
