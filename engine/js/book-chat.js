...
export function openBookChat(opts = {}) {
  ...
  if (opts.term) {
    ...
    const runExplain = () => {
      appendUser(`Explain: ${term} (hint tier ${tier})`);
      sendChat({
        term,
...
async function sendChat({ message, term, subject, tier, bookFigure }) {
  ...
  if (term) {
    if (tierN <= 1) {
      userContent = `Give ONE short nudge sentence (max 20 words) about "${term}" in ${subject || chatState.subject}. No full definition yet.`;
    } else if (tierN === 2) {
      userContent = `Explain "${term}" simply in 2-3 short sentences for kids, context: ${subject || chatState.subject}.`;
    } else {
      userContent = `Give a direct clear answer for "${term}" in ${subject || chatState.subject}, then point to a book diagram if relevant${
  ...
  const payload = {
    subject: subject || chatState.subject,
    term: term || undefined,
  ...
  try {
    ...
    status?.remove();
    if (!res.ok) {
      // TODO: ensure GROQ_API_KEY in .env when proxy returns 503
      appendBot(
        data.error
    ...
    status?.remove();
    appendBot("Ask your teacher — chat is offline right now.");
  } finally {
