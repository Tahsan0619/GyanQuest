...
export function mountAuthControls(host) {
  ...
  const paint = () => {
    const user = getUser();
    host.innerHTML = user
      ? `<span class="gq-auth__who">${escapeHtml(user.name || user.email)}</span>
         <button type="button" class="btn secondary" id="gq-auth-logout">Log out</button>`
      : `<button type="button" class="btn secondary" id="gq-auth-open">Log in</button>`;
  ...
  if (!root) {
    ...
    root.innerHTML = `
      ...
      <div class="gq-auth-card" role="dialog" aria-modal="true" aria-label="Account">
        ...
        <label>Name (register)<input id="gq-auth-name" type="text" autocomplete="name" /></label>
        <label>Email<input id="gq-auth-email" type="email" autocomplete="username" /></label>
        <label>Password<input id="gq-auth-pass" type="password" autocomplete="current-password" /></label>
        ...
        <div class="gq-auth-actions">
          ...
          <button type="button" class="btn secondary" id="gq-auth-register">Register</button>
          <button type="button" class="btn primary" id="gq-auth-login">Log in</button>
        </div>
      </div>`;
...
function ensureAuthCss() {
  ...
  link.rel = "stylesheet";
  link.href = "/engine/css/auth.css?v=1";
