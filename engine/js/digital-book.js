...
export function linkTerms(html, glossary = []) {
  let out = String(html || "");
  // Longer terms first; each glossary term is marked at most once per page chunk.
  ...
  for (const g of terms) {
    ...
    out = out.replace(re, (m, _g1, offset, whole) => {
      // Skip if this match sits inside an existing term button.
      const before = whole.slice(0, offset);
      if (before.lastIndexOf("<button") > before.lastIndexOf("</button>")) return m;
